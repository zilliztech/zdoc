...
- On a `next` deployment, NEXT pages render normally with an unreleased-feature banner in the site's locale. On a `current` deployment, NEXT pages render the shared 404 page (the same component the docs plugin mounts for misses under `/docs`), disappear from sidebars and DocCard lists, carry `noindex`, are excluded from the sitemap, `llms.txt`, structured data, and `.md` build copies, and the routes listed in `release-channel-routes.txt` are additionally blocked with Nginx 404s that serve the styled 404 page.
