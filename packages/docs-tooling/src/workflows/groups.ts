...
function chineseGroupOrder(): readonly string[] {
  return Object.freeze([
    'guides',
    ...manualRegistry
      // `rest` is a reference manual but its Chinese content is spec-generated by
      // the fetch flow's publish_rest_zh-CN job (not Lark-fetched nor translation), so
      // it must remain a resolvable zh-CN source publication group.
      .filter(manual => (manual.id === 'rest' || manual.kind !== 'reference') && manual.id !== 'guides' && manual.id !== 'guides-byoc' && manual.publications['zh-CN'])
      .map(manual => manual.id),
  ]);
}
