...
Vector search orders results by vector distance, but the initial order may not reflect how well each candidate's text answers the query. With a [Hugging Face model provider integration](./hugging-face), Hugging Face Ranker uses scores from the Hugging Face sentence-similarity task to reorder the candidates returned by vector search.
...
<table>
   ...
   <tr>
     ...
     <td><p>Yes</p></td>
     <td><p>The reranking implementation. Set this value to <code>model</code>.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>Yes</p></td>
     <td><p>The Hugging Face Model ID for a model currently served through <code>hf-inference</code> for the <code>sentence-similarity</code> task.</p></td>
   </tr>
   <tr>
     <td><p><code>queries</code></p></td>
     <td><p>Yes</p></td>
     <td><p>A list of query texts used for reranking. Provide one string for each search query (<code>nq</code>), even when the initial search uses query vectors.</p></td>
   </tr>
   <tr>
   ...
   <tr>
     ...
     <td><p>No</p></td>
     <td><p>The maximum number of candidate texts sent to Hugging Face in one request. The default value is <code>32</code>. The value must be greater than <code>0</code>.</p></td>
   </tr>
</table>
