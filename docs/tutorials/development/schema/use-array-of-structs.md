...
- You need document-level results from the Embedding List search, or Struct-element-level results from the element-level search.
...
If your answers to the questions above are yes, you should use **StructArray**.
...
Keep the combined number of vector fields at the collection level and inside StructArray fields no greater than the upper bound of your cluster. For details, refer to [Zilliz Cloud Limits](./limits).  
...
- **Nullable**
Fields inside a StructArray are not nullable.

- **Function**
You cannot use a function to derive a vector field from a scalar field within a Struct.

- **Index type & metric type**
All vector fields in a collection must be indexed, including vector sub-fields inside a StructArray. StructArray vector sub-fields support two search modes:  

    - Embedding List search uses query data in EmbeddingList form and a `MAX_SIM` metric. Supported Embedding List metrics are `MAX_SIM`, `MAX_SIM_COSINE`, `MAX_SIM_IP`, `MAX_SIM_L2`, . `MAX_SIM` is equivalent to `MAX_SIM_COSINE`.

    - Element-level search uses a regular vector metric supported by the element vector type, such as `COSINE`, `L2`, or `IP` for dense float or int vectors, and .

    - If you need both search modes, define separate vector sub-fields and create one index on each sub-field. A single vector sub-field can have only one index configuration, so you cannot create one `MAX_SIM_*` index and one regular-metric index on the same sub-field. For Zilliz Cloud, use `AUTOINDEX` for both Embedding List search and element-level search. For details on how Zilliz Cloud calculates the similarity between the query and an embedding list, refer to [Maximum Similarity](./search-metrics-explained#maximum-similarity). For details on StructArray search modes, refer to [Search with StructArray](./search-with-structarray).

    - The scalar fields in the StructArray field support the following index types:

        - `INVERTED`

            This usually applies to string-like or categorical filters, like `structA[color]` or `structA[str_val]`. For details, refer to [INVERTED](./inverted-index-type).

        - `STL_SORT`

            This usually applies to range or order-style acceleration on numeric values, like `structA[num_val]`. For details, refer to [STL_SORT](./slt-sort-index-type).

- **Upsert data**
Structs do not support upsert in merge mode. However, you can still perform upserts in override mode to update data in Structs. For details about the differences between upsert in merge mode and override mode, refer to [Upsert Entities](./upsert-entities).
...
typeParams.put("mmap_enabled", "true");
collectionSchema.addField(AddFieldReq.builder()
        .fieldName("chunks")
...
schema := entity.NewSchema().
    ...
    WithField(entity.NewField().WithName("author").WithDataType(entity.FieldTypeVarChar).WithMaxLength(512)).
    WithField(entity.NewField().WithName("year_of_publication").WithDataType(entity.FieldTypeInt64)).
    WithField(entity.NewField().WithName("title_vector").WithDataType(entity.FieldTypeFloatVector).WithDim(5))
...
structSchema := entity.NewStructSchema().
    ...
    WithField(entity.NewField().WithName("text_vector").WithDataType(entity.FieldTypeFloatVector).WithDim(5).WithTypeParams("mmap_enabled", "true"))
...
### Index an embedding list\{#index-an-embedding-list}
...
indexOptions := []milvusclient.CreateIndexOption{
    ...
    milvusclient.NewCreateIndexOption(
        "my_collection",
        "chunks[text_vector]",
        index.NewAutoIndex(entity.MaxSimCosine),
    ).WithIndexName("chunks_text_vector_index"),
    // highlight-end
}
```
...
### Index a scalar struct sub-field\{#index-a-scalar-struct-sub-field}

When you create indexes on a scalar struct sub-field, Zilliz Cloud clusters actually builds the index at the **element level**, not at the row level, to accelerate scalar filtering.
...
```go
indexOptions = append(indexOptions,
    milvusclient.NewCreateIndexOption(
        "my_collection",
        "chunks[text]",
        index.NewInvertedIndex(),
    ).WithIndexName("chunks_text_index"),
)
```
...
zilliz vector search \
  --collection my_collection \
  --data "[$embeddingList1]" \
  --anns-field "chunks[text_vector]" \
  --limit 3 \
  --output-fields '["chunks[text]"]'
```
...
```python
filter = 'element_filter(chunks, $[text] LIKE "Red%")'
```
...
```python
# correct
filter = 'id > 0 && element_filter(chunks, $[x] > 1)'

# incorrect, resulting errors
filter = 'element_filter(chunks, $[x] > 1) && id > 0'
```
...
- `MATCH_LEAST(chunks, $[text] LIKE "Red%", threshold=k)`This returns entities that contain at least `k` chunks that start with "Red" in the `text` sub-field.

- `MATCH_MOST(chunks, $[text] LIKE "Red%", threshold=k)`This returns entities that contain at most `k` chunks that start with "Red" in the `text` sub-field.

- `MATCH_EXACT(chunks, $[text] LIKE "Red%", threshold=k)`This returns entities that contain exactly `k` chunks that start with "Red" in the `text` sub-field.
