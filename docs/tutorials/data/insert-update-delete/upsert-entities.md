...
beta: FALSE
notebook: FALSE
description: "The `upsert` operation provides a convenient way to insert or update entities in a collection. It intelligently handles data by checking for the existence of a primary key if the key already exists, the corresponding entity is updated; otherwise, a new entity is inserted. This makes `upsert` the recommended method for managing data when you are unsure if an entity already exists or when you need to avoid creating duplicate entries. | Cloud"
type: origin
