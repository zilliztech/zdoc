...
The `decompounder` filter is a [custom filter](./analyzer-overview) in Zilliz Cloud. To use it, specify `"type": "decompounder"` in the filter configuration, along with a `word_list` parameter that provides the dictionary of word components to recognize.
