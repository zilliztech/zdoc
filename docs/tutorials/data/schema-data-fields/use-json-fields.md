...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud allows you to store and index structured data within a single field using the `JSON` data type. This enables flexible schemas with nested attributes while still allowing efficient filtering via JSON path indexing. | Cloud"
type: origin
