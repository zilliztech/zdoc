...
On Zilliz Cloud, a serverless cluster indicates that the compute resources are provisioned dynamically on-demand, and the user who has created the cluster shares the resources with other serverless cluster users. This type of cluster eliminates the need to provision and manage [compute unit (CU)](./cu-types-explained-1) resources, as Zilliz Cloud takes care of the underlying infrastructure.
