...
- `data` (optional): The metadata field to preserve. The input field name should be consistent with what you defined when [creating the Ingestion pipeline](./create-ingestion-pipelines) and adding the __PRESERVE__ function. The value of this field should also follow the predefined field type.
