...
1. Edit `terraform.tfvars.example` with your details and rename it to `terraform.tfvars`. The configuration includes the AWS region, AWS access and secret keys, and the external ID for your BYOC organization in Zilliz Cloud.
    - `aws_region`: The AWS region where you want to deploy Zilliz Cloud services. Currently, the BYOC license supports only AWS **us-east-2 **region.
...
- **Management Role and Policy**: These are utilized for regular management tasks of the BYOC in your AWS account, limiting operations to resources tagged **BYOC **in Zilliz Cloud.

- **Bootstrap Role and Policy**: These are employed for initial setup, teardown, and urgent operations. After successful cloud activation, these can be safely removed. These roles work exclusively with resources tagged **BYOC** and cannot create or modify new IAM policies.
