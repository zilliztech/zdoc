...
- **Vector field**
    ...
    - SPARSE_FLOAT_VECTOR: Stores a list of non-zero elements and their corresponding indices, used for representing sparse vectors. When using `SPARSE_FLOAT_VECTOR`, you do not need to specify the dimension.

    - BINARY_VECTOR: Stores binary data as a sequence of 0s and 1s, used for compact feature representation in image processing and information retrieval. When using `BINARY_VECTOR`, note that the dimension must be a multiple of 8, ranging from 8 to 32,768 * 8.
