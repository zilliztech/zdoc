...
Zilliz Cloud enables hybrid search capabilities using the [hybrid_search()](/reference/python/python/Collection-hybrid_search) API, incorporating sophisticated reranking strategies to refine search results from multiple `AnnSearchRequest` instances. This topic covers the reranking process, explaining its significance and implementation of different reranking strategies in Zilliz Cloud.
...
## Weighted Scoring (WeightedRanker){#weighted-scoring-weightedranker}
...
## Reciprocal Rank Fusion (RRFRanker){#reciprocal-rank-fusion-rrfranker}
