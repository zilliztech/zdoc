...
The `stemmer` filter is a [custom filter](./analyzer-overview) in Zilliz Cloud. To use it, specify `"type": "stemmer"` in the filter configuration, along with a `language` parameter to select the desired language for stemming.
