...
The `length` filter is a [custom filter](./analyzer-overview) in Zilliz Cloud. To use it, specify `"type": "stop"` in the filter configuration, along with a `stop_words` parameter that provides a list of stop words.
