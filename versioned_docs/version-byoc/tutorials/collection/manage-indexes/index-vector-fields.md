...
Zilliz Cloud employs [AUTOINDEX](./autoindex-explained) to enable efficient similarity searches, supporting both vector searches and scalar filtering. It offers three [metric types](./search-metrics-explained): __Cosine Similarity__ (COSINE), __Euclidean Distance__ (L2), and __Inner Product__ (IP) to measure the distances between vector embeddings.
