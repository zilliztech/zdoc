...
- You have created a collection. To create a collection, refer to [Manage Collections (SDKs)](./manage-collections-sdks#create-collection).
...
Successful insertion of entities into a collection requires that the provided data aligns with the pre-defined schema of the collection.

To learn more about the concepts of the schema and entities, refer to [Schema Explained](./schema-explained) and [Cluster, Collection & Entities](./cluster-collection-and-entities).
...
<ul>
<li><p>Do not include <code>id</code> in the data to be inserted, because <code>auto_id</code> is enabled.</p></li>
<li><p>Fields other than <code>id</code> and <code>vector</code> are treated as dynamic fields, because <code>enable_dynamic_field</code> is enabled.</p></li>
</ul>
...
### Insert into partitions{#insert-into-partitions}
...
### Upsert data in partitions{#upsert-data-in-partitions}
