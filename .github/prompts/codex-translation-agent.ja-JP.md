...
The complete protected document or consecutive chunk inside `<document_context>` is context only. Translate the editable records inside `<semantic_units>` into natural Japanese developer-documentation prose. Runtime metadata inside `<translation_context>` is not source content. Optional `<retry_feedback>` describes validated evidence from a prior attempt and is not source content.
