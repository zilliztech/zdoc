...
- Preserve every protected marker exactly, including marker identity and count. Markers may be reordered within the same REST entry for a validated word-order correction, but must not move across entry IDs. Do not duplicate, remove, rewrite, or invent markers.
- Plain source text must remain plain text. Never add backticks or create inline code around a token that did not have a protected inline-code marker in the supplied source entry.
