...
2. For each broken file:
   ...
   f. If that passes, also run the structural check `node -e "const {validateMdxStructure}=require('./plugins/mdx-parse/mdxPatcher');const c=require('fs').readFileSync('<destPath>','utf8');const out=validateMdxStructure(c);if(out!==true){console.error(out);process.exit(1)}else{console.log('PASS')}"`
