...
notebook: FALSE
description: "As a collection moves from development to production, the fields around each entity often change. You might add scalar fields such as `sourceuri` or `reviewstatus` for filtering and application logic, add a new vector field for embeddings generated by your application, add a BM25-generated sparse vector field for lexical search over existing text, or remove fields that are no longer used. Alter Collection Schema lets you make supported field changes in place instead of recreating the collection. | BYOC"
type: origin
...
- [Add vector fields generated by functions](./add-fields-to-an-existing-collection#add-vector-fields-generated-by-functions) when Zilliz Cloud should generate vector values from existing fields, such as BM25 sparse vectors or MinHash signatures from text.
...
### Add vector fields generated by functions\{#add-vector-fields-generated-by-functions}
...
- A generated vector output field, such as `text_sparse`, that stores values generated by Zilliz Cloud.
...
| --- | --- | --- |
| `BM25` | `SPARSE_FLOAT_VECTOR` | A `VARCHAR` or `TEXT` field with analyzer enabled |
| `MINHASH` | `BINARY_VECTOR` | A `VARCHAR` or `TEXT` field |
...
**Example: Add a BM25-generated sparse vector field**

The following example adds a BM25-generated sparse vector field named `text_sparse` to an existing collection. The collection must already have a `VARCHAR` text field named `text` with analyzer enabled.
...
Conceptually, adding a BM25-generated sparse field contains the following schema changes:
...
After the request succeeds, `describe_collection()` returns both the new `text_sparse` field and the `text_bm25` function in the collection schema. For the complete BM25 search workflow, refer to [Full Text Search](./full-text-search).

Zilliz Cloud also supports MinHash-generated binary vector fields for near-duplicate detection. A MinHash function uses `FunctionType.MINHASH` and writes to a new `BINARY_VECTOR` output field. For configuration details, refer to [MinHash Function](./minhash-function).

## Drop fields from an existing collection\{#drop-fields-from-an-existing-collection}

You can drop fields from an existing collection in two ways. Drop user-defined scalar or vector fields directly when they are no longer part of your collection model. Drop fields generated by functions by dropping the function that generates them.
...
### Drop vector fields generated by functions\{#drop-vector-fields-generated-by-functions}
...
To remove a generated vector field, call `drop_collection_function()` on the function that generates it. In this workflow, Zilliz Cloud removes the function from the collection schema and also removes its generated vector output fields.
...
**Example: Drop a field generated by a function**
...
### Which add-field method should I use?\{#which-add-field-method-should-i-use}
...
Use the generated-vector-field workflow when Zilliz Cloud should produce vector values from existing fields. This guide shows the BM25 path with `add_function_field()` for lexical search. Zilliz Cloud also supports MinHash-generated binary vector fields for near-duplicate detection.
...
### Can I add BM25 lexical search to an existing collection?\{#can-i-add-bm25-lexical-search-to-an-existing-collection}

Yes. If the collection already has a `VARCHAR` or `TEXT` field with analyzer enabled, you can add a BM25-generated sparse vector field for lexical search. In this workflow, Zilliz Cloud adds the new `SPARSE_FLOAT_VECTOR` output field and the BM25 function that generates values for it.
...
### Can I drop a vector field generated by a function directly?\{#can-i-drop-a-vector-field-generated-by-a-function-directly}

No. A vector field generated by a function is part of that function's schema contract. Use `drop_collection_function()` instead. In this schema-change workflow, Zilliz Cloud removes the function and its generated vector output fields together, while preserving the input fields.
