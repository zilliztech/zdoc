...
| **Analyzer component** | **Parameter that accepts a file resource** |
| --- | --- |
...
| **Type** | **Where the file lives** | **Who manages the file** | **Fit** |
| --- | --- | --- | --- |
| **Remote** | In the object store (MinIO / S3 / GCS / Azure) that your Milvus cluster is already configured to use | Milvus, via the `add_file_resource` / `remove_file_resource` / `list_file_resources` client APIs | Recommended for most deployments. |
| **Local** | At the same absolute path on the local filesystem of every Milvus component (DataNode, QueryNode, StreamingNode) | You — mount the file yourself, for example via a Kubernetes volume | Open-source / self-hosted scenarios where you prefer to manage dictionary files outside Milvus. |
