...
test('push runs create an external link progress card before checking', () => {
  assert.match(workflow, /id: push_card[\s\S]*github\.event_name == 'push'[\s\S]*pnpm docs-tooling report-card create[\s\S]*--title "External Link Check"[\s\S]*--stages "Check external links"/)
})

test('pull requests create a card only when new broken links exist', () => {
  assert.match(workflow, /id: pr_card[\s\S]*github\.event_name == 'pull_request'[\s\S]*steps\.summary\.outputs\.has_new_broken == 'true'[\s\S]*pnpm docs-tooling report-card create/)
})
...
test('summary and card finalization run after checker failure without replacing its result', () => {
  ...
  assert.match(workflow, /name: Finish Feishu card[\s\S]*if: \$\{\{ always\(\)[\s\S]*pnpm docs-tooling report-card finish/)
