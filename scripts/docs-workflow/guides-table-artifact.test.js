...
const identity = { site: 'en', table_id: 'tbl-tools', table_name: 'Tools', table_slug: 'tools', target: 'zilliz.saas', target_name: 'saas', cleanup: false }
...
test('creates and validates a table-scoped artifact', async () => {
  ...
  await fs.mkdir(path.join(workspace, 'tmp/docs-tooling/en/guides/content/en/guides/tutorials/tools/agents'), { recursive: true })
  await fs.writeFile(path.join(workspace, 'tmp/docs-tooling/en/guides/content/en/guides/tutorials/tools/agents/agent.md'), 'agent')
  ...
  assert.deepEqual(manifest.files.map(file => file.path), ['tmp/docs-tooling/en/guides/content/en/guides/tutorials/tools/agents/agent.md'])
...
test('rejects artifact payload paths outside the owned table directory', async () => {
  const workspace = await fs.mkdtemp(path.join(os.tmpdir(), 'guides-table-artifact-'))
  const output = path.join(workspace, 'artifact')
  await fs.mkdir(path.join(workspace, 'tmp/docs-tooling/en/guides/content/en/guides/tutorials/tools'), { recursive: true })
  await fs.writeFile(path.join(workspace, 'tmp/docs-tooling/en/guides/content/en/guides/tutorials/tools/page.md'), 'page')
  ...
  manifest.files[0].path = 'tmp/docs-tooling/en/guides/content/en/guides/tutorials/management/escape.md'
