  ...
  for (const file of files) {
    ...
    if (file === '_translate-content-group.yml') {
      ...
      const targetValidation = /validate-mdx --path i18n\/ja-JP[\s\S]*validate-translation --target ja-JP --group "\$GROUP"[\s\S]*build:en/.test(unbatchedRun)
        && /if \[\[ "\$GROUP" == reference-landings \]\]; then[\s\S]*validate-mdx --path content\/zh-CN\/reference\/api\/python\/python\/python\.md --write[\s\S]*validate-mdx --path content\/zh-CN\/reference\/api\/java\/java\/java\.md --write[\s\S]*validate-mdx --path content\/zh-CN\/reference\/api\/nodejs\/nodejs\/nodejs\.md --write[\s\S]*validate-mdx --path content\/zh-CN\/reference\/api\/go\/go\/go\.md --write[\s\S]*validate-mdx --path content\/zh-CN\/reference\/cli\/cli\/Overview\.md --write[\s\S]*fi[\s\S]*validate-mdx --path content\/zh-CN\/reference --check[\s\S]*reference-manifest --write[\s\S]*validate-reference --site zh-CN[\s\S]*build:zh-CN/.test(unbatchedRun)
