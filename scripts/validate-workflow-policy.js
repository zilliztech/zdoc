...
function validateWorkflowPolicies(directory = workflowDirectory, options = {}) {
  ...
  for (const file of files) {
    ...
    if (file === '_prepare-translation-batches.yml') {
      const requiredPatterns = [
        ...
        [/restore-generated-state\.sh --exact --ref "\$SOURCE_CHECKPOINT_SHA" --except "\$restore_except"/, 'must keep target-side translation state on the target baseline while restoring the source checkpoint'],
        [/diff -r "\$GITHUB_WORKSPACE\/\$parity_root" "\$target_baseline_dir\/\$parity_root"/, 'must verify target-side baseline parity before deterministic batching'],
    ...
    if (file === '_translate-content-group.yml') {
      ...
      const requiredPatterns = [
        ...
        [/restore-generated-state\.sh --exact --ref "\$SOURCE_CHECKPOINT_SHA" --except "\$restore_except"/, 'must keep target-side translation state on the target baseline while restoring the source checkpoint'],
        [/diff -r "\$GITHUB_WORKSPACE\/\$parity_root" "\$BASELINE_DIR\/\$parity_root"/, 'must verify target-side baseline parity before reconciliation'],
  ...
  for (const [pattern, message] of [
    ...
    [/offline candidate contains a forbidden \$\{status\} change/, 'must reject undeclared deletions and special candidate changes'],
    [/orphan deletion source still exists at reconciliation source checkpoint/, 'must bind every offline orphan deletion to a source absent from the reconciliation source checkpoint'],
    [/offline candidate deletions must exactly match the reconciliation plan/, 'must bind every offline deletion to the authenticated reconciliation plan'],
    [/replacement must preserve the exact Japanese translation bytes/, 'must preserve exact Japanese bytes across canonical replacements'],
