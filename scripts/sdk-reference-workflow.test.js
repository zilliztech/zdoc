...
test('docs workflow orchestrates parallel producers and checkpointed sequential publication', () => {
  ...
  assert.match(source, /tooling_ref:[\s\S]*description: Artifact-only tooling ref override[\s\S]*type: string[\s\S]*default: master/)
  ...
  assert.match(source, /if \[\[ "\$PUBLISH" == true \]\]; then\n\s+tooling_ref=master/)
