...
test('docs workflow orchestrates independent checkpointed publication lanes', () => {
  ...
  assert.equal(workflow.jobs.produce_guides_sources.with.card_id, '${{ needs.prepare.outputs.phase_card_id }}')
  assert.equal(workflow.jobs.produce_guides_sources.with.card_mode, '${{ needs.prepare.outputs.card_mode }}')
