...
test('watchdog pins tooling and content identities before the exact rendered scan', () => {
  ...
  const buildChinese = stepNamed('Build Chinese site')
  ...
  assert.equal(buildChinese.run, 'pnpm build:zh-CN')
  assert.ok(steps.indexOf(build) < steps.indexOf(buildChinese))
  assert.equal(scan.id, 'scan')
  assert.match(scan.run, /check-links --site en --output tmp\/external-link-watchdog\/en\.md/)
  assert.match(scan.run, /check-links --site zh-CN --output tmp\/external-link-watchdog\/zh-CN\.md/)
  assert.ok(steps.indexOf(buildChinese) < steps.indexOf(scan))
...
test('scan exposes a validated count and uploads the complete report directory', () => {
  const scan = stepNamed('Scan rendered external links')
  assert.match(scan.run, /reports = \[.*en.*zh-CN/)
  assert.ok(scan.run.includes('tmp/external-link-watchdog/${site}.json'))
  assert.match(scan.run, /Number\.isSafeInteger\(expiredCount\)/)
  assert.match(scan.run, /count < 0/)
...
test('every successful scan sends one bounded report card with expiry-derived presentation', () => {
  ...
  assert.match(note.run, /--input tmp\/external-link-watchdog\/en\.json/)
  assert.match(note.run, /--input tmp\/external-link-watchdog\/zh-CN\.json/)
