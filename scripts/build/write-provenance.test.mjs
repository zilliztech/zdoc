...
test('Chinese candidate workspace ignores Japanese inputs owned by the other site', () => {
  ...
  assert.doesNotThrow(() => runZh(root, {environment: candidateEnvironment()}));
});

test('candidate workspace accepts English sidebars regenerated for the current source release', () => {
  const root = fixture();
  ...
  write(root, 'generated/en/sidebars/cli.sidebar.js', 'module.exports = ["current-cli"]\n');
  execFileSync('git', ['add', 'generated/en/sidebars/cli.sidebar.js'], {cwd: root});
...
test('Japanese candidate workspace ignores Chinese inputs owned by the other site', () => {
  const root = fixture();
  ...
  write(root, relativePath, '# Changed independently by the Chinese site\n');
  assert.doesNotThrow(() => run(root, {environment: candidateEnvironment({
    ZDOC_PROVENANCE_CANDIDATE_TARGET: 'ja-JP',
  })}));
});

test('candidate workspace accepts regenerated English sidebars that differ from the dev baseline', () => {
  const root = fixture();
  write(root, 'generated/en/sidebars/cli.sidebar.js', 'module.exports = ["published-cli"]\n');
  execFileSync('git', ['add', '.'], {cwd: root});
  execFileSync('git', ['commit', '-qm', 'published English source checkpoint'], {cwd: root});
