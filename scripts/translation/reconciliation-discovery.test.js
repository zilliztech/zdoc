...
test('shares canonical bidirectional mappings and exact group ownership', () => {
  ...
  assert.equal(mapSourcePathForTarget('zh-CN-reference', 'content/en/guides/tutorials/home.md'), 'content/zh-CN/guides/tutorials/home.md')
  assert.equal(mapTargetPathForSource('zh-CN-reference', 'content/zh-CN/guides/tutorials/home.md'), 'content/en/guides/tutorials/home.md')
})
...
test('preserves declared English landing pages and ignores cross-group paths', () => {
  ...
  const checkpoint = commit(repository, 'checkpoint')
  const result = discover(repository, {sourceBaselineSha: baseline, sourceCheckpointSha: checkpoint, targetBaselineSha: baseline}, {target: 'ja-JP'})
  assert.deepEqual(result.candidates, [])
})
