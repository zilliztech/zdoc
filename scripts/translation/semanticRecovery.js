...
function serializeRecoverySemanticCheckpoints(checkpoints, item) {
  return serializeSemanticCheckpoints(checkpoints, item)
}
...
function semanticCheckpointBytes(value) {
  ...
  if (!report) return 0
  return report.entries?.reduce((total, entry) => total + Buffer.byteLength(entry?.translation || ''), 0) || 0
}
