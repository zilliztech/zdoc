...
test('guides workflows bootstrap full sources and persist only verified caches', () => {
  ...
  assert.equal(sourceSteps[publicationPreflightIndex].env.IMAGE_BED_URL, '${{ vars.IMAGE_BED_URL }}')
  ...
  assert.equal(publicationStep.env.IMAGE_BED_URL, "${{ inputs.site == 'zh-CN' && vars.IMAGE_BED_URL || '' }}")
