...
test('translation workers resolve bootstrap mode and validate only their selected group', () => {
  ...
  const regenerateIndex = steps.findIndex(step => step.name === 'Regenerate Chinese Reference manifests and sidebars')
  ...
  assert.match(steps[regenerateIndex].run, /reference-manifest[\s\S]*--source-commit "\$SOURCE_COMMIT_SHA"[\s\S]*--write/)
