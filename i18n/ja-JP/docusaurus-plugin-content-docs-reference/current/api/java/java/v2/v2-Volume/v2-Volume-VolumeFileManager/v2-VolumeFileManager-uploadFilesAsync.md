...
notebook: false
description: "再試行、同時実行性、マルチパート、および進捗レポートを設定可能な形で、ファイルまたはディレクトリを Zilliz Cloud volume に非同期でアップロードします。 | Java | v2"
type: docx
...
再試行、同時実行性、マルチパート、および進捗レポートを設定可能な形で、ファイルまたはディレクトリを Zilliz Cloud volume に非同期でアップロードします。
...
- `partSizeBytes(long partSizeBytes)`
    ...
    マルチパートアップロードのパートサイズ（バイト単位）です。0 以下の値を指定すると自動サイズ設定が有効になります。
