...
notebook: false
description: "この操作は、前回の flush-all アクションが完了したかどうかを確認します。`flushAll` を非同期で呼び出し、完了までポーリングする必要がある場合に使用します。 | Java | v2"
type: docx
...
この操作は、前回の flush-all アクションが完了したかどうかを確認します。`flushAll` を非同期で呼び出し、完了までポーリングする必要がある場合に使用します。
