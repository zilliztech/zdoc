...
notebook: false
description: "InstructorEmbeddingFunction におけるこの操作は、テキスト文字列のリストを受け取り、それらを直接ベクトル埋め込みにエンコードします。 | Python"
type: docx
...
[InstructorEmbeddingFunction](./EmbeddingModels-InstructorEmbeddingFunction) におけるこの操作は、テキスト文字列のリストを受け取り、それらを直接ベクトル埋め込みにエンコードします。

InstructorEmbeddingFunction の **\_\_call\_\_()** メソッドは、[encode_documents()](./InstructorEmbeddingFunction-encode_documents) および [encode_queries()](./InstructorEmbeddingFunction-encode_queries) と同じ機能を共有します。
