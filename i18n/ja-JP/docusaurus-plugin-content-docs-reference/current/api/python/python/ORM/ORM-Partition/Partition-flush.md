...
notebook: false
description: "この操作は、パーティション内のすべてのセグメントをシールします。この操作以降の挿入は、新しいセグメントを生成します。 | Python | ORM"
type: docx
...
この操作は、パーティション内のすべてのセグメントをシールします。この操作以降の挿入は、新しいセグメントを生成します。
...
新しいデータが挿入されると、そのデータは成長中のセグメントに書き込まれます。成長中のセグメントのサイズが上限に達すると、Zilliz Cloud は自動的にそのセグメントをシールします。
