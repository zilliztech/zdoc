...
- **batch_size** (*int*) -
    ...
    各イテレーションで返すエンティティの数。デフォルト値は 1000 です。

- **limit** (*int*) -

    返すエンティティの総数。このパラメーターの値は 16,384 未満である必要があります。 
...
- **timeout** (*float* | *None*) -
    ...
    この操作のタイムアウト時間。これを **None** に設定すると、いずれかの応答が到着するか、何らかのエラーが発生した時点でこの操作がタイムアウトすることを示します。
...
- **kwargs** -
    ...
    - **consistency_level** (*str* | *int*) -
        ...
        デフォルト値は、現在のコレクションを作成したときに指定した値で、**Strong** (**0**)、**Bounded** (**1**)、**Session** (**2**)、**Eventually** (**3**) から選択できます。

        <Admonition type="info" title="Note">

        整合性レベルとは何ですか？
        
                分散データベースにおける整合性とは、特定の時点でデータの書き込みや読み取りを行う際に、すべてのノードまたはレプリカが同じデータビューを持つことを保証する性質を指します。
        
                Zilliz Cloud は、**Strong**、**Bounded Staleness**、**Eventually** の 3 つの整合性レベルを提供しており、デフォルトは **Bounded Staleness** です。
        
                ベクトル類似検索やクエリを実行する際に、アプリケーションに最適となるように整合性レベルを簡単に調整できます。
    ...
    - **guarantee_timestamp** (*int*) -
        ...
        このパラメーターを設定すると、このタイムスタンプより前に挿入されたすべてのエンティティがクエリノードから参照可能である場合にのみ、この操作はクエリを実行します。 

        <Admonition type="info" title="Notes">

        このパラメーターは、デフォルトの整合性レベルが適用される場合に有効です。
    ...
    - **graceful_time** (*int*) -
        ...
        デフォルト値は **5** です。このパラメーターを設定すると、この値を現在のタイムスタンプから差し引いて保証タイムスタンプを計算します。

        <Admonition type="info" title="Notes">

        このパラメーターは、デフォルト以外の整合性レベルが適用される場合に有効です。
    ...
    - **limit** (*int*) -
        ...
        このパラメーターを `offset` と組み合わせて使用すると、ページネーションを有効にできます。
...
以下のメソッドを提供する **QueryIterator** インスタンスです。
...
- **DataTypeNotMatchException**
    ...
    この例外は、パラメーターの値が必要なデータ型と一致しない場合に発生します。
