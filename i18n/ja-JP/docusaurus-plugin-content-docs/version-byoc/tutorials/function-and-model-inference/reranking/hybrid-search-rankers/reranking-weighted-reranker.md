...
beta: FALSE
notebook: FALSE
description: "Weighted Ranker は、複数の検索パスに対して異なる重要度の重みを割り当てることで、結果をインテリジェントに統合し優先順位付けします。熟練したシェフが複数の食材をバランスよく調合して完璧な料理を作り上げるのと同様に、Weighted Ranker も異なる検索結果を調整し、最も関連性の高い統合された成果を提供します。このアプローチは、複数のベクトルフィールドやモダリティにまたがって検索を行う場合で、特定のフィールドが最終的なランキングにより大きく寄与すべき場合に最適です。| BYOC"
type: origin
