...
notebook: FALSE
description: "Zilliz Cloudは、特に大規模なデータセットにおいてフィルタリングと検索性能を著しく高速化するために、スカラーフィールド（非ベクトルフィールド）のインデックス作成をサポートしています。| BYOC"
type: origin
...
Zilliz Cloudは、特に大規模なデータセットにおいてフィルタリングと検索性能を著しく高速化するために、スカラーフィールド（非ベクトルフィールド）のインデックス作成をサポートしています。

## 概要\{#overview}

スカラーフィールドのインデックス作成は任意ですが、フィルター条件で特定のスカラーフィールドを頻繁にアクセスする場合は推奨されます。
...
client = MilvusClient(uri="YOUR_CLUSTER_ENDPOINT") # 自分のクラスターエンドポイントに置き換えてください

# 動的フィールドサポートを含むスキーマを定義
...
const client = new MilvusClient({
  address: 'YOUR_CLUSTER_ENDPOINT', // 自分のクラスターエンドポイントに置き換えてください
});
...
## スカラーフィールドにインデックスをかける\{#index-a-scalar-field}

`AUTOINDEX`を使用してスカラーフィールドにインデックスを作成できます。追加のインデックスパラメータは必要ありません。以下の例では、`price`フィールドにインデックスを作成しています。

<Tabs groupId="code" defaultValue='python' values={[{"label":"Python","value":"python"},{"label":"Java","value":"java"},{"label":"NodeJS","value":"javascript"},{"label":"Go","value":"go"},{"label":"cURL","value":"bash"}]}>
<TabItem value='python'>

```python
index_params = client.prepare_index_params() # 任意のインデックスパラメータを指定せずに空のIndexParamsオブジェクトを準備

index_params.add_index(
    field_name="price", # インデックスをかけるスカラーフィールドの名前
    # highlight-next-line
    index_type="AUTOINDEX", # 作成するインデックスの種類
    index_name="price_index" # 作成するインデックスの名前
)
```
...
スカラーインデックスに関連するいくつかの高度な機能もあります。
