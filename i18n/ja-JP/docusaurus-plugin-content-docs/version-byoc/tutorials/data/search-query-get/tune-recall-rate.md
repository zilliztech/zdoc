...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloudは、ユーザーが検索リコールとパフォーマンスのバランスを取るための検索パラメータ`level`を導入しました。また、現在の検索の推定リコール率をユーザーに提供するための別の検索パラメータ`enablerecallcalculation`も提供しています。これら2つのパラメータを組み合わせて、ベクトル検索のリコール率を調整することができます。 | BYOC"
type: origin
