...
beta: FALSE
notebook: FALSE
description: "ANN検索では、指定されたベクトル埋め込みに最も似たベクトル埋め込みが見つかります。ただし、検索結果が常に正しいとは限りません。検索要求にフィルタリング条件を含めることで、ZillizクラウドANN検索を実行する前にメタデータフィルタリングを実行し、指定されたフィルタリング条件に一致するエンティティのみに検索範囲を縮小します。 | BYOC"
type: origin
