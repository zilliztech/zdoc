...
beta: FALSE
notebook: FALSE
description: "分散ベクトルデータベースとして、Zillizクラウド各ノードまたはレプリカが読み取りおよび書き込み操作中に同じデータにアクセス可能であることを確認するために、複数の一貫性レベルを提供します。現在、サポートされている一貫性レベルには、Strong、Bounded、Eventally、およびSessionが含まれ、Boundedがデフォルトの一貫性レベルとして使用されています。 | BYOC"
type: origin
