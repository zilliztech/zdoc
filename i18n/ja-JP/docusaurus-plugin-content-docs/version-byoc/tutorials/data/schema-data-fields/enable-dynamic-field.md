...
beta: FALSE
notebook: FALSE
description: "Zillizクラウド「ダイナミックフィールド」と呼ばれる特別な機能を通じて、柔軟で進化する構造を持つエンティティを挿入することができます。このフィールドは、`$meta`という隠しJSONフィールドとして実装されており、コレクションスキーマで明示的に定義されていないデータ内のフィールドを自動的に保存します。 | BYOC"
type: origin
