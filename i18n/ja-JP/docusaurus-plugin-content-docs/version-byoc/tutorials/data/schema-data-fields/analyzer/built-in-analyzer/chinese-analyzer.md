...
notebook: FALSE
description: "`chinese`アナライザーは、中国語テキストを効果的に処理するように特別に設計され、効果的なセグメンテーションとトークン化を提供します。| BYOC"
type: origin
...
`chinese`アナライザーは、中国語テキストを効果的に処理するように特別に設計され、効果的なセグメンテーションとトークン化を提供します。

### 定義\{#definition}

`chinese`アナライザーは以下の要素で構成されています：
...
`chinese`アナライザーの機能は、以下のカスタムアナライザー構成と同等です：
...
<p><code>chinese</code>アナライザーは任意のオプションパラメータを受け入れません。</p>
