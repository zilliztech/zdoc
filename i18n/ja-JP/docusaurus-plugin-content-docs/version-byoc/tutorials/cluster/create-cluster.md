...
## 作成するクラスタ{#create-alessinclude-targetsaasgreater-dedicatedlessincludegreater-cluster}
...
1. [新しいクラスタの作成]ページで、関連するパラメータを入力します。
    ...
    - **クラウドプロバイダーの設定**:クラウドサービスプロバイダーとクラスターが展開される特定のリージョンを選択してください。BYOCライセンスでは、現在AWSのus-west-2リージョンのみがサポートされています。より多くのクラウドリージョンをリクエストするには、[お問い合わせ](https://zilliz.com/cloud-region-request?firstname=Li&lastname=Yun&company=zilliz&name=zilliz&email=leryn.li@zilliz.com&fullname=Li%20Yun&phone=--&country=China&requested_csp_provider=AWS)をクリックしてください。
...
次の`POST`リクエストはリクエストボディを受け取り、1つのPerformance-optimized[CU](./cu-types-explained)を持つ`cluster-02`という名前のクラスター。
