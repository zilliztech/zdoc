...
## ストレージロールの権限{#storage-role-permissions}{#storage-role-permissions}
...
## EKSロールの権限{#eks-role-permissions}{#ekseks-role-permissions}

Zilliz CloudでEKSクラスターを管理するために、次の権限を持つEKSロールを作成しました。

### AWSが管理する権限{#aws-managed-permissions}{#awsaws-managed-permissions}
...
### Kubernetes SIGsからの権限{#permissions-from-kubernetes-sigs}{#kubernetes-sigspermissions-from-kubernetes-sigs}
...
## クロスアカウントのロール権限{#cross-account-role-permissions}{#cross-account-role-permissions}
