...
例えば、`age`と`city`の2つのフィールドを含むフィルター式があり、年齢が大なり25歳で「北京」または「上海」に住んでいるすべての人を見つけたいとします。フィルター式に値を直接埋め込む代わりに、テンプレートを使用することができます
...
## 検索オペレーション{#search-operations}{#search-operations}
...
## クエリ操作{#query-operations}{#query-operations}
...
## 削除操作{#delete-operations}{#delete-operations}
