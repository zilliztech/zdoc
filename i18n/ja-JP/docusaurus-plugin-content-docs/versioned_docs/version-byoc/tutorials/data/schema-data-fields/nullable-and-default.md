...
## Nullableな属性{#nullable-attribute}{#nullablenullable-attribute}
...
### エンティティを挿入{#insert-entities}{#insert-entities}
...
### NULL値を使用した検索とクエリ{#search-and-query-with-null-values}{#nullsearch-and-query-with-null-values}
...
## デフォルト値{#default-values}{#default-values}

デフォルト値は、スカラーフィールドに割り当てられたプリセット値です。挿入時にデフォルトのフィールドに値を指定しない場合、システムは自動的にデフォルト値を使用します。

### デフォルト値を設定{#set-default-values}{#set-default-values}
...
### デフォルト値を使用した検索とクエリ{#search-and-query-with-default-values}{#search-and-query-with-default-values}
...
## 適用ルール{#applicable-rules}{#applicable-rules}
