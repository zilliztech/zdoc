---
title: "VectorDBBenchによるパフォーマンスベンチマーク | Cloud"
slug: /perf-benchmark-vectordb
sidebar_label: "VectorDBBenchによるパフォーマンスベンチマーク"
beta: FALSE
notebook: FALSE
description: "VectorDBBenchは、ベクトルデータベースに特化したオープンソースのベンチマークツールです。 | Cloud"
type: origin
...
## ベンチマーク指標{#benchmark-metrics}{#benchmark-metrics}
...
- [少なくとも1つのクラスタを作成](/docs/create-cluster)してください。Zilliz Cloudは、[無料](./free-trials)のクラスタを提供しています。これにより、Zilliz Cloudベクトルデータベースをすばやく利用できます。

- Python 3.11以降がインストールされている必要があります。

## 手続き{#procedures}{#procedures}

### テスト環境の設定{#set-up-testing-environment}{#set-up-testing-environment}
...
### VectorDBBenchのインストールと起動{#install-and-start-vectordbbench}{#vectordbbenchinstall-and-start-vectordbbench}
...
### ベンチマークテストを設定する{#configure-your-benchmarking-test}{#configure-your-benchmarking-test}

### ベンチマーク結果を見る{#view-benchmarking-results}{#view-benchmarking-results}
