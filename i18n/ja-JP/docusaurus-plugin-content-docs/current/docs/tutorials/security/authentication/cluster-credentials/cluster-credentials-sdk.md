...
デフォルトの`db_admin`ユーザ以外にも、認証用の対応するパスワードを持つクラスタユーザ[を作成](./cluster-users#)することもできます。

## パスワードのリセット{#}

ユーザーのパスワードを忘れた場合や漏洩の疑いがある場合は、パスワードをリセットできます。

![reset-cluster-user-password](/img/ja-JP/reset-cluster-user-password.png)