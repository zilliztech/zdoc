...
一括で属性を共有する複数のエンティティを削除する場合、フィルタ式を使用できます。以下のサンプルコードは、**in**演算子を使用して、**カラー**フィールドの値が**赤**と**緑**に設定されたすべてのエンティティを一括削除します。他の演算子を使用して、要件を満たすフィルタ式を作成することもできます。フィルタ式の詳細については、Filteringを参照してください。
