...
生データをJSONファイルに準備するには、[BulkWriterツールを](null)使用することをお勧めします。次の図は、生データをJSONファイルにマップする方法を示しています。
...
- [複数のパスからファイルをインポートする（推奨）](./data-import-json#)

- [フォルダからファイルをインポートする](./data-import-json#)

- [単一のファイルをインポート](./data-import-json#)
...
「[データファイルの準備](https://milvus.io/docs/bulk_insert.md#Prepare-the-data-file)」を参照して、自分でデータを再構築するか、[BulkWriterツールを](null)使用してソースデータファイルを生成できます。[上の図のスキーマに基づいて準備されたサンプルデータをダウンロードするには、ここをクリックしてください](https://assets.zilliz.com/prepared_json_data.json)。