...
生データをParquetファイルに準備するには、[BulkWriterツールを](null)使用することをお勧めします。次の図は、生データをParquetファイルにマッピングする方法を示しています。
...
- [複数のパスからファイルをインポートする（推奨）](./data-import-parquet#)

- [ソースフォルダからファイルをインポートする](./data-import-parquet#)

- [単一のファイルをインポート](./data-import-parquet#)
...
生データをパルケファイルに準備するに[はBulkWriterツール](null)を使用することをお勧めします。[上の図のスキーマに基づいて準備されたサンプルデータをダウンロードするにはここをクリック](https://assets.zilliz.com/prepared_parquet_data.parquet)してください。