...
### Zilliz Cloudクラスタのセットアップ{#zilliz-cloud}

- まだ作成していない場合は、[クラスタを作成し](null)ます。
...
### RemoteBulkWriterの作成{#remotebulkwriter}
...
行ディクショナリには、すべてのスキーマ定義フィールドをキーとして含める必要があります。動的フィールドが許可されている場合は、未定義フィールドも含めることができます。詳細は、Use BulkWriterするを参照してください。
...
<p>データとターゲットコレクションがAWSによってホストされている場合、オブジェクトURLは<strong>s 3://remote-bucket/file-path</strong>に似ている必要があります。ライターによって返されたデータパスのプレフィックスに適用可能なURIについては、「Storage Options」を参照してください。</p>
