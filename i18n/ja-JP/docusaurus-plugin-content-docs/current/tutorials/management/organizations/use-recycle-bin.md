...
## Recycle Bin で削除されたクラスターを復元する\{#restore-a-dropped-cluster-in-the-recycle-bin}
...
1. 復元するクラスターを見つけます。**Actions** ドロップダウンから **Restore Full クラスター** を選択します。

1. 復元するクラスターを設定します。

    1. クラスターはこの組織内の別のプロジェクトに復元できますが、別のクラウドリージョンには復元できません。
...
1. 新しい復元ジョブが生成されます。[Jobs](./job-center) ページでクラスター復元の進行状況を確認できます。ジョブステータスが **IN PROGRESS** から **SUCCESSFUL** に変わると、復元は完了です。
