...
1. 既存のアラートを変更するには、対象のアラートターゲットの横にある **Actions** 列から **Edit** を選択します。新規にアラートを作成する場合は、右上の **+ Alert** をクリックします。
    ...
    組織アラートでは既存のアラートターゲットのみ編集可能で、新規作成はサポートされていません。詳細については、[Manage Organization Alerts](./manage-organization-alerts) を参照してください。
...
1. [Create an Events API v2 integration](https://support.pagerduty.com/docs/services-and-integrations#create-a-generic-events-api-integration) を実行してインテグレーションキーを取得します。キーの形式は `c55ec4de243e440bd0e921750bdfxxxx` です。
...
1. [Zilliz Cloud コンソール](https://cloud.zilliz.com/signup) で Webhook 通知チャネルを設定します。
    ...
    1. 既存のアラートを編集するには、対象のアラートターゲットの横にある **Actions** 列から **Edit** を選択します。新規アラートを作成する場合は、右上の **+ Alert** をクリックします。
    ...
    1. **Alert Resolution Notification** および **Enable Alert** で、アラートの解決時またはトリガー時に実行するアクションをそれぞれ設定します。
...
1. 作成したボットの情報を確認し、該当ボットの Webhook URL を取得します。詳しい手順については、[グループボットの Webhook アドレスの取得](https://open.work.weixin.qq.com/help2/pc/14931?person_id=1&searchData=#%E4%BA%94%E3%80%81%E7%BE%A4%E6%9C%BA%E5%99%A8%E4%BA%BAWebhook%E5%9C%B0%E5%9D%80) を参照してください。
...
1. 作成したボットの情報を確認し、該当ボットの Webhook URL を取得します。詳しい手順については、[カスタムボットの Webhook アドレスの取得](https://open.dingtalk.com/document/orgapp/obtain-the-webhook-address-of-a-custom-robot) を参照してください。
