...
notebook: FALSE
description: "Zilliz Cloud では、クラスターユーザーを作成し、クラスターロールを割り当てて権限を定義することで、データのセキュリティを確保できます。 | Cloud"
type: origin
...
このガイドでは、クラスターユーザーの作成、ユーザーへのロールの付与、ユーザーからのロールの取り消し、およびユーザーの削除について説明します。クラスターロールの詳細については、[クラスターロールの管理 (コンソール)](./cluster-roles) を参照してください。

## ユーザーの作成\{#create-a-user}

次の例は、ユーザー名 `user_1`、パスワード `P@ssw0rd` でユーザーを作成する方法を示しています。ユーザー名とパスワードは、以下の規則に従う必要があります。

- ユーザー名: 英字で始まり、大文字・小文字の英字、数字、アンダースコアのみ使用できます。

- パスワード: 8〜64文字で、大文字の英字、小文字の英字、数字、特殊文字のうち3種類以上を含む必要があります。

- 説明 (任意): 最大1024バイト (UTF-8) です。
...
以下は出力例です。`root` は自動生成されるデフォルトユーザーです。`user_1` は作成したばかりの新規ユーザーです。
...
ユーザーに割り当て済みのロールを取り消すこともできます。

次の例は、ユーザー `user_1` に割り当てられているロール `role_a` を取り消す方法を示しています。
...
ユーザーを削除したら、既存のすべてのユーザーを一覧表示して、削除操作が成功したかどうかを確認できます。
...
以下は出力例です。一覧に `user_1` が含まれていなければ、削除操作は成功しています。
