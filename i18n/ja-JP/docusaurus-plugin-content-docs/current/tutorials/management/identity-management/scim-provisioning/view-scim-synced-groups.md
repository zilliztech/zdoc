...
SCIM 同期グループは Zilliz Cloud では読み取り専用です。Zilliz Cloud で SCIM 同期グループを直接作成したり、ユーザーを追加・招待したり、メンバーシップを変更したりすることはできません。グループ名、メンバーシップ、ライフサイクルは IdP で管理してください。IdP はそれらの変更を SCIM 経由で Zilliz Cloud に同期します。

## 事前準備\{#before-you-start}

- [SCIM プロビジョニングの概要](./scim-provisioning-overview) を確認し、Zilliz Cloud 組織の SCIM プロビジョニングを設定済みであること。

- IdP が少なくとも 1 つのグループを Zilliz Cloud にプロビジョニング済みであること。

## 同期グループの表示\{#view-synced-groups}

次のインタラクティブデモでは、Zilliz Cloud で SCIM 同期グループの一覧を開く方法を示しています。
