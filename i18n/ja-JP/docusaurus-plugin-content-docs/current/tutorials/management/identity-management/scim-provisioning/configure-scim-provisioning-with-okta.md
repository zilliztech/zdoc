---
title: "Okta を使用した SCIM プロビジョニングの設定 | Cloud"
...
notebook: FALSE
description: "このガイドでは、Okta から Zilliz Cloud への SCIM プロビジョニングの設定方法について説明します。SCIM プロビジョニングを使用すると、Okta から Zilliz Cloud 組織へユーザー、グループ、グループメンバーシップをプッシュできます。 | Cloud"
type: origin
...
1. **SCIM Base URL** と **SCIM API Token** をコピーします。これらの値は、Okta で API 統合を設定する際に使用します。
...
1. アプリケーションの名前（例: `Zilliz Cloud SCIM`）を入力し、**Next** をクリックします。
...
1. Okta SCIM アプリで **Provisioning** タブを開き、**Configure API Integration** をクリックします。
...
1. Okta が Zilliz Cloud に送信するプロビジョニングアクションを設定します。
...
1. Okta SCIM アプリで **Assignments** タブを開きます。

1. **Assign** をクリックし、**Assign to People** を選択します。

1. 割り当てるユーザーを検索し、**Assign** をクリックします。

1. ユーザー属性を確認し、**Save and Go Back** をクリックします。

1. **Assigned** ステータスを確認し、**Done** をクリックします。
...
1. Okta SCIM アプリで **Push Groups** タブを開きます。

1. **Push Groups** をクリックし、**Find groups by name** を選択します。

1. Zilliz Cloud にプッシュするグループを選択します。
