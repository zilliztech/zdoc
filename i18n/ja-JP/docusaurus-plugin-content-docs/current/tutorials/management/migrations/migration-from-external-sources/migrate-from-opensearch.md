...
notebook: FALSE
description: "このトピックでは、OpenSearch から移行する際に、Zilliz Cloud がデータ型マッピング、collection 命名規則、および考慮事項をどのように扱うかを説明します。 | Cloud"
type: origin
...
# OpenSearch から Zilliz Cloud へ移行する

このトピックでは、[OpenSearch](https://opensearch.org/) から移行する際に、Zilliz Cloud がデータ型マッピング、collection 命名規則、および考慮事項をどのように扱うかを説明します。
...
| データの可用性 | ソース index にはデータが含まれている必要があります。空の index は移行できません。 |
...
| cluster 容量 | 十分なストレージおよびコンピュートリソース（[CU calculator](https://zilliz.com/pricing#calculator) を使用して CU サイズを見積もってください） |
| ネットワークアクセス | ネットワーク制限を使用している場合は、許可リストに [Zilliz Cloud IPs](./zilliz-cloud-ips) を追加してください |

## データ型マッピング\{#data-type-mapping}

次の表は、OpenSearch のフィールド型が Zilliz Cloud のフィールド型にどのようにマッピングされるかをまとめたものです。また、カスタマイズオプションの詳細も示しています。

| **OpenSearch フィールド型** | **Zilliz Cloud フィールド型** | **説明** |
| --- | --- | --- |
| Primary key | Primary key | OpenSearch の primary key ([_id](https://opensearch.org/docs/latest/field-types/metadata-fields/id/)) は、Zilliz Cloud では自動的に primary key としてマッピングされます。<br/>データの移行時に Auto ID を有効にできます。ただし、有効にすると、ソーステーブルの元の primary key 値は破棄されます。 |
| [k-NN vector](https://opensearch.org/docs/latest/field-types/supported-field-types/knn-vector/) | FLOAT_VECTOR | OpenSearch の `float` vector 型は、Zilliz Cloud では `FLOAT_VECTOR` にマッピングされます。OpenSearch の Byte/Binary vector は移行ではサポートされていません。<br/>vector の次元数は変更されません。 |
...
| [Boolean](https://opensearch.org/docs/latest/field-types/supported-field-types/boolean/) | BOOL | `true` または `false` を保存します。 |
| [Date](https://opensearch.org/docs/latest/field-types/supported-field-types/dates/) | VARCHAR | 文字列として保存されます。正しい形式変換を行ってください。 |
...
| シナリオ | 影響 | 解決策 |
| --- | --- | --- |
| デフォルト命名 | Collection 名はソース index 名と完全に一致します | 名前は OpenSearch からそのまま保持されます |
| 特殊文字 | ハイフン (-) またはドット (.) を含む index 名はエラーの原因となり、ジョブ送信を妨げます | アンダースコアまたはその他の有効な文字を使用するように index 名を手動で変更してください |
...
| Sparse vector | 現行リリースではサポートされていません | dense vector の代替案を検討するか、ロードマップについてサポートにお問い合わせください |
