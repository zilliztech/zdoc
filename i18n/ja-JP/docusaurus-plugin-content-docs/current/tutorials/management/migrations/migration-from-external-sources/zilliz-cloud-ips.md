---
title: "Zilliz Cloud IP アドレス | Cloud"
...
notebook: FALSE
description: "Zilliz Cloud は固定 IP アドレスのセットを使用して、データソースからデータを取得し、ターゲットクラスターに送信します。これを確実に行えるようにするには、ファイアウォールでこれらの IP アドレスをセーフリストに追加する必要があります | Cloud"
type: origin
...
Zilliz Cloud は固定 IP アドレスのセットを使用して、データソースからデータを取得し、ターゲットクラスターに送信します。これを確実に行えるようにするには、ファイアウォールで以下の IP アドレスをセーフリストに追加する必要があります。

- Zilliz Cloud サービス IP アドレス

- ターゲットクラスターが配置されているクラウドリージョンの IP アドレス

## Zilliz Cloud サービス IP アドレス\{#zilliz-cloud-service-ip-addresses}

これらの IP アドレスは、Zilliz Cloud がサービスを提供するために使用されます。これらをファイアウォールの許可リストに追加してください。
...
| ap-northeast-1 | 日本東京 | `35.72.252.126` |
...
| ap-northeast-2 | 韓国ソウル | `43.200.197.223`, `54.116.65.71`, `43.200.30.237`, `13.125.81.204` |
...
| Central India | インド、プネー | `98.70.222.135` |
