...
notebook: FALSE
description: "このガイドでは、サブスクリプション手順をステップごとに説明し、GCP Marketplace 上の Zilliz Cloud の料金体系について概説します。 | Cloud"
type: origin
...
サブスクライブ後、Google Cloud Marketplace を通じて Google Cloud cluster の利用料金を支払えます。ほかのクラウドプロバイダー上にデプロイされた cluster がある場合も、Google Cloud Marketplace を使用して支払うことができます。
...
- GCP Marketplace account が organization に属している場合、billing administrator から購入権限を付与されている必要があります。

## GCP Marketplace でサブスクライブする\{#subscribe-on-gcp-marketplace}

[GCP](https://console.cloud.google.com/marketplace)[ Marketplace](https://console.cloud.google.com/marketplace) にアクセスし、以下の手順で Zilliz Cloud のサブスクライブを開始します。
...
1. サブスクリプション用の project を選択し、**Subscribe** をクリックします。 
...
1. 新しいタブで、以下の手順に従ってサブスクリプションを完了します。
    ...
    1. すでに Zilliz Cloud account を持っている場合は、そのままログインします。持っていない場合は、[サインアップオプション](./register-with-zilliz-cloud) を選択して手順に従ってください。
...
サブスクリプションをキャンセルすると、organization は高度な Zilliz Cloud 機能へのアクセスを失います。organization に残っているクレジットがない場合、またはすべてのクレジットの有効期限が切れている場合は、直ちに凍結されます。
...
**Marketplace subscription を Zilliz Cloud にリンクする際、利用可能な organization がない場合はどうすればよいですか？**
...
- **権限不足** 
    ...
    marketplace subscription に organization をリンクするには、**Organization Owner** または **Organization Billing Admin** である必要があります。Organization Member のみである場合は、必要な権限がありません。organization owner に連絡してサポートを受けてください。
...
- **すべての organization がすでに Marketplace subscription に正常にリンクされている**
    ...
    - 既存の marketplace subscription を更新する必要がある場合は、まず organization の現在のサブスクリプションのリンクを解除してから、新しいサブスクリプションを設定してください。
...
- **リストに organization がない**
    ...
    - account が閉鎖されている場合、またはすべての organization から退出している場合に発生することがあります。UI は以下のようになります。
