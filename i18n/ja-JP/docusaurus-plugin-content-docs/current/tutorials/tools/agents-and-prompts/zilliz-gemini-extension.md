...
Gemini CLI 用の Zilliz Cloud 拡張機能は、Zilliz Cloud の操作を IDE に直接持ち込む自然言語インターフェースです。CLI コマンドを覚えたり Web コンソールに切り替えたりする代わりに、やりたいことを平易な言葉で説明するだけで、プラグインが処理します。
...
または、先に [このリポジトリ](https://github.com/zilliztech/gemini-cli-extension.git) をローカルにクローンし、次のコマンドを実行することもできます。
...
1. クラスターに接続します。
    ...
    または、プラグインに利用可能なクラスターからの選択を手伝ってもらうこともできます。
