...
notebook: FALSE
description: "Information Retrieval システム（検索エンジンとも呼ばれる）は、Retrieval-augmented generation (RAG)、ビジュアル検索、製品レコメンデーションなど、さまざまな AI アプリケーションに不可欠です。これらのシステムの中核には、情報を整理し、インデックス化し、取得するために慎重に設計されたデータモデルがあります。 | Cloud"
type: origin
...
Information Retrieval システム（検索エンジンとも呼ばれる）は、Retrieval-augmented generation (RAG)、ビジュアル検索、製品レコメンデーションなど、さまざまな AI アプリケーションに不可欠です。これらのシステムの中核には、情報を整理し、インデックス化し、取得するために慎重に設計されたデータモデルがあります。

Zilliz Cloud では、collection schema を通じて検索データモデルを指定でき、非構造化データ、その dense または sparse な vector 表現、および構造化メタデータを整理できます。テキスト、画像、またはその他のデータ型を扱う場合でも、このハンズオンガイドは、実際に検索データモデルを設計するための主要な schema の概念を理解し、適用するのに役立ちます。

![Kc3Cweq1AhAmMGbrVgRcTlTKnUf](https://zdoc-images.s3.us-west-2.amazonaws.com/Kc3Cweq1AhAmMGbrVgRcTlTKnUf.png)

## データモデル\{#data-model}

検索システムのデータモデル設計では、ビジネスニーズを分析し、情報を schema で表現されたデータモデルへ抽象化します。明確に定義された schema は、データモデルをビジネス目標に整合させ、データの一貫性とサービス品質を確保するうえで重要です。さらに、適切なデータ型と index を選択することは、ビジネス目標を経済的に達成するうえで重要です。

### ビジネスニーズの分析\{#analyzing-business-needs}

ビジネスニーズに効果的に対応するには、まずユーザーが実行するクエリの種類を分析し、最適な検索方法を判断することから始めます。

- **ユーザークエリ:** ユーザーが実行すると想定されるクエリの種類を特定します。これにより、schema が実際のユースケースをサポートし、検索パフォーマンスを最適化できるようになります。これには次のようなものがあります。

    - 自然言語クエリに一致するドキュメントを取得する

    - 参照画像に類似する画像、またはテキスト説明に一致する画像を見つける
...
- **検索方法:** ユーザーが実行するクエリの種類に合った適切な検索技術を選択します。異なる方法は異なる目的に対応しており、より強力な結果を得るために組み合わせることもよくあります。

    - **セマンティック検索**: dense vector の類似度を使用して、意味が類似した項目を見つけます。テキストや画像などの非構造化データに最適です。

    - **全文検索**: キーワードマッチングによってセマンティック検索を補完します。全文検索では語彙解析を利用して長い単語が断片化されたトークンに分割されるのを避け、検索時に特殊な用語を把握できます。
...
- 保存する必要があるデータを定義します。たとえば、生コンテンツ（テキスト、画像、音声）、関連メタデータ（タイトル、タグ、著者情報）、コンテキスト属性（タイムスタンプ、ユーザー行動など）です。

- 各要素に対して適切なデータ型と形式を決定します。たとえば次のようになります。
...
これらの要素を明確に定義することで、データの一貫性、正確な検索結果、および下流のアプリケーションロジックとの統合の容易さが確保されます。

## Schema 設計\{#schema-design}

Zilliz Cloud では、データモデルは collection schema を通じて表現されます。collection schema 内で適切な field を設計することは、効果的な検索を実現する鍵です。各 field は collection に保存される特定の種類のデータを定義し、検索プロセスにおいてそれぞれ異なる役割を果たします。大まかには、Zilliz Cloud は **vector fields** と **scalar fields** の 2 つの主要な field タイプをサポートしています。

これで、vector と補助的な scalar fields を含め、データモデルを field の schema にマッピングできます。各 field がデータモデルの属性に対応していることを確認し、特に vector type（dense または spase）とその dimension に注意してください。

### Vector Field\{#vector-field}

vector fields は、テキスト、画像、音声などの非構造化データ型に対する embedding を格納します。これらの embedding は、データ型と使用される検索方法に応じて、dense、sparse、または binary になります。通常、dense vectors はセマンティック検索に使用され、sparse vectors は全文検索または語彙マッチングにより適しています。binary vectors は、ストレージや計算リソースが限られている場合に有用です。collection には、マルチモーダルまたはハイブリッド検索戦略を可能にするために複数の vector fields を含めることができます。このトピックの詳細なガイドについては、[Multi-Vector Hybrid Search](./hybrid-search) を参照してください。
...
scalar fields は、数値、文字列、日付など、一般にメタデータと呼ばれるプリミティブで構造化された値を格納します。これらの値は vector 検索結果と一緒に返すことができ、フィルタリングやソートに不可欠です。これにより、特定のカテゴリや定義済みの時間範囲にドキュメントを限定するなど、特定の属性に基づいて検索結果を絞り込むことができます。

Zilliz Cloud は、`BOOL`、`INT8/16/32/64`、`FLOAT`、`DOUBLE`、`VARCHAR` などの scalar 型に加え、非 vector データの保存とフィルタリングのための `JSON` や `ARRAY` などの composite 型もサポートしています。これらの型は、検索操作の精度とカスタマイズ性を高めます。

## Schema 設計で高度な機能を活用する\{#leverage-advanced-features-in-schema-design}

schema を設計する際、サポートされているデータ型を使って単にデータを field にマッピングするだけでは不十分です。field 間の関係や、設定に利用できる戦略を深く理解することが重要です。設計段階で重要な機能を念頭に置くことで、その schema は目先のデータ処理要件を満たすだけでなく、将来のニーズに対してもスケーラブルかつ適応可能になります。これらの機能を慎重に統合することで、Zilliz Cloud の能力を最大限に引き出し、より広範なデータ戦略と目標を支える強固なデータアーキテクチャを構築できます。以下は、collection schema を作成する際の主要機能の概要です。

### Primary Key\{#primary-key}

primary key field は schema の基本的な構成要素であり、collection 内の各エンティティを一意に識別します。primary key の定義は必須です。これは整数型または文字列型の scalar field であり、`is_primary=True` としてマークする必要があります。必要に応じて、primary key に `auto_id` を有効化できます。これにより、collection にデータが取り込まれるにつれて単調増加する整数が自動的に割り当てられます。

詳細については、[Primary Field & AutoID](./primary-field-auto-id) を参照してください。

### Partitioning\{#partitioning}

検索を高速化するために、必要に応じて partitioning を有効にできます。特定の scalar field を partitioning 用に指定し、検索時にその field に基づくフィルタ条件を指定することで、検索範囲を関連する partition のみに効果的に限定できます。この方法により、検索対象ドメインが縮小され、検索処理の効率が大幅に向上します。

詳細については、[Use Partition Key](./use-partition-key) を参照してください。

### Analyzer\{#analyzer}

analyzer はテキストデータを処理・変換するための重要なツールです。その主な機能は、生テキストをトークンに変換し、インデックス作成と検索のために構造化することです。これは、文字列をトークン化し、ストップワードを除去し、個々の単語を語幹化してトークンにすることで実現されます。

詳細については、[Analyzer Overview](./analyzer-overview) を参照してください。

### Function\{#function}

Zilliz Cloud では、特定の field を自動的に導出するために、schema の一部として組み込み function を定義できます。たとえば、`VARCHAR` field から sparse vector を生成する組み込み BM25 function を追加して、全文検索をサポートできます。こうした function 由来の field により前処理が簡素化され、collection が自己完結かつクエリ可能な状態に保たれます。

詳細については、[Full Text Search](./full-text-search) を参照してください。

## 実世界の例\{#a-real-world-example}

このセクションでは、上の図に示したマルチメディアドキュメント検索アプリケーションの schema 設計とコード例を示します。この schema は、以下の field にマッピングされるデータを持つ記事データセットを管理するために設計されています。

| **Field** | **Data Source** | **Used By Search Methods** | **Primary Key** | **Partition Key** | **Analyzer** | **Function Input/Output** |
| --- | --- | --- | --- | --- | --- | --- |
| article_id (`INT64`) | `auto_id` を有効化して自動生成 | [Query using Get](./get-and-scalar-query) | Y | N | N | N |
...
schema に関する詳細情報と、さまざまな種類の field を追加するための詳しいガイダンスについては、[Schema Explained](./schema-explained) を参照してください。

### ステップ 1: schema を初期化する\{#step-1-initialize-schema}

まず、空の schema を作成する必要があります。このステップで、データモデルを定義するための基盤構造を確立します。
...
### ステップ 3: （任意）function を追加する\{#step-3-optional-add-functions}

データクエリ機能を強化するために、schema に function を組み込むことができます。たとえば、特定の field に関連する処理を行う function を作成できます。
...
この例では、schema に組み込み BM25 function を追加し、`text` field を入力として使用し、結果の sparse vector を `text_sparse_vector` field に保存します。
