...
| --- | --- | --- |
| スキーマ定義 | コレクションスキーマで `DataType.JSON` 型として明示的に宣言する必要があるスカラーフィールド | 宣言されていないフィールドを自動的に保存する、非表示の JSON フィールド（`$meta` という名前） |
| 用途 | スキーマが既知で一貫している構造化データを保存します。 | 固定スキーマに適合しない、柔軟で進化する、または半構造化されたデータを保存します。 |
| 制御 | フィールド名と構造はユーザーが制御できます。 | 未定義のフィールドはシステムが管理します。 |
| クエリ | フィールド名または JSON フィールド内の対象キーを使用してクエリします：`metadata["key"]` | dynamic field のキーを直接使用してクエリします：`"dynamic_key"`、または `$meta` 経由：`$meta["dynamic_key"]` |
...
| NGRAM Index | ワイルドカード検索、テキストフィールドでの部分文字列一致 | N/A | numeric/range フィルター向けではない |
