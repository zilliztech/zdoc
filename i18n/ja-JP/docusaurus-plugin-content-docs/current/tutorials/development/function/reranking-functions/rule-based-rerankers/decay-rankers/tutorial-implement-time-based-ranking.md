...
notebook: FALSE
description: "多くの検索アプリケーションでは、コンテンツの新しさは関連性と同じくらい重要です。ニュース記事、商品リスト、ソーシャルメディア投稿、研究論文はいずれも、意味的関連性と新しさのバランスを取るランキングシステムの恩恵を受けます。このチュートリアルでは、decay ranker を使用して Zilliz Cloud で時間ベースのランキングを実装する方法を示します。 | Cloud"
type: origin
...
多くの検索アプリケーションでは、コンテンツの新しさは関連性と同じくらい重要です。ニュース記事、商品リスト、ソーシャルメディア投稿、研究論文はいずれも、意味的関連性と新しさのバランスを取るランキングシステムの恩恵を受けます。このチュートリアルでは、decay ranker を使用して Zilliz Cloud で時間ベースのランキングを実装する方法を示します。

## decay ranker を理解する\{#understand-decay-rankers}

decay ranker を使用すると、基準点に対する数値（タイムスタンプなど）に基づいてドキュメントをブーストまたはペナルティできます。時間ベースのランキングでは、意味的関連性が似ている場合でも、新しいドキュメントに古いドキュメントより高いスコアを与えることができます。

Zilliz Cloud は 3 種類の decay ranker をサポートしています。

- **Gaussian** (`gauss`): なだらかで滑らかな減衰を提供するベル型曲線

- **Exponential** (`exp`): 新しいコンテンツを強く重視するために、初期の急激な減衰を作成

- **Linear** (`linear`): 予測しやすく理解しやすい直線的な減衰

各 ranker には異なる特性があり、さまざまなユースケースに適しています。詳細については、[Decay Ranker の概要](./decay-ranker-oveview) を参照してください。

## 時間を考慮した検索システムを構築する\{#build-a-time-aware-search-system}

関連性と時間の両方に基づいてコンテンツを効果的にランク付けする方法を示すニュース記事検索システムを作成します。まずは実装から始めましょう。
...
高速な vector 検索のために適切なインデックスパラメータを設定します。
...
このチュートリアルでは、異なる公開日を持つニュース記事のセットを作成します。decay ranking の効果を明確に示すため、内容がほぼ同一で日付だけが異なる記事のペアを含めている点に注目してください。
...
次に、それぞれの違いを際立たせるため、異なるパラメータを持つ 3 種類の decay ranker を作成します。
...
- `reranker`: 時間ベースの減衰関数として `decay` に設定

- `function`: 減衰関数の種類（gauss、exp、または linear）

- `origin`: 基準点（通常は現在時刻）

- `offset`: ドキュメントが完全な関連性を維持する期間

- `scale`: offset を超えた後に関連性がどれだけ速く低下するかを制御

- `decay`: offset+scale における減衰係数（例: `0.5` は関連性が半分であることを意味します）

これらの関数を異なる挙動に調整できることを示すために、exponential ranker には異なるパラメータを設定している点に注目してください。
...
次に、検索クエリを実行し、decay ranking ありとなしで結果を比較してみましょう。
...
## Step 10: 時間減衰を用いたハイブリッド検索\{#step-10-hybrid-search-with-time-decay}
...
## Step 11: 異なるパラメータ値を試す\{#step-11-experiment-with-different-parameter-values}

scale パラメータを調整すると Gaussian 減衰関数にどのような影響があるかを見てみましょう。
...
Milvus の減衰関数を使用した時間ベースのランキングは、意味的関連性と新しさのバランスを取るための強力な方法を提供します。適切な減衰関数とパラメータを設定することで、意味的関連性を維持しながら新しいコンテンツを優先表示する検索体験を作成できます。

このアプローチは特に次のような用途で有効です。
...
減衰関数の背後にある数式を理解し、さまざまなパラメータを試すことで、特定のユースケースにおいて関連性と鮮度の最適なバランスを実現するように検索システムを微調整できます。
