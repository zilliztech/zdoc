...
notebook: FALSE
description: "多くの検索アプリケーションでは、コンテンツの鮮度は関連性と同じくらい重要です。ニュース記事、商品リスト、ソーシャルメディアの投稿、研究論文はいずれも、意味的な関連性と新しさのバランスを取るランキングシステムの恩恵を受けます。このチュートリアルでは、decay ranker を使用して Zilliz Cloud で時間ベースのランキングを実装する方法を示します。 | Cloud"
type: origin
...
多くの検索アプリケーションでは、コンテンツの鮮度は関連性と同じくらい重要です。ニュース記事、商品リスト、ソーシャルメディアの投稿、研究論文はいずれも、意味的な関連性と新しさのバランスを取るランキングシステムの恩恵を受けます。このチュートリアルでは、decay ranker を使用して Zilliz Cloud で時間ベースのランキングを実装する方法を示します。

## decay ranker を理解する\{#understand-decay-rankers}

decay ranker を使うと、基準点に対する数値（タイムスタンプなど）に基づいてドキュメントをブーストまたはペナルティできます。時間ベースのランキングでは、意味的な関連性が似ている場合でも、新しいドキュメントの方が古いドキュメントより高いスコアを得られることを意味します。

Zilliz Cloud は 3 種類の decay ranker をサポートしています。

- **Gaussian** (`gauss`): 滑らかで緩やかな減衰を提供するベル型曲線

- **Exponential** (`exp`): 新しいコンテンツを強く重視するために、初期の低下をより急峻にします

- **Linear** (`linear`): 予測しやすく理解しやすい直線的な減衰

各 ranker には異なる特性があり、さまざまなユースケースに適しています。詳細については、[Decay Ranker Overview](./decay-ranker-oveview) を参照してください。

## 時間を考慮した検索システムを構築する\{#build-a-time-aware-search-system}

関連性と時間の両方に基づいてコンテンツを効果的にランキングする方法を示すニュース記事検索システムを作成します。まずは実装から始めましょう。
...
時間ベースの検索では、コンテンツとともに公開タイムスタンプも保存する必要があります。
...
## ステップ 3: index パラメータを構成する\{#step-3-configure-index-parameters}

高速な vector 検索のために適切な index パラメータを設定しましょう。
...
このチュートリアルでは、公開日が異なるニュース記事のセットを作成します。decay ranking の効果を明確に示すために、内容がほぼ同一で日付だけが異なる記事のペアを含めている点に注目してください。
...
## ステップ 5: 異なる decay ranker を構成する\{#step-5-configure-different-decay-rankers}

次に、違いが際立つように、それぞれ異なるパラメータを持つ 3 つの decay ranker を作成します。
...
- `reranker`: 時間ベースの decay 関数には `decay` を設定します

- `function`: decay 関数の種類（gauss、exp、または linear）
...
異なる挙動に合わせてこれらの関数をどのように調整できるかを示すために、exponential ranker には異なるパラメータを設定している点に注目してください。
...
それでは、検索クエリを実行し、decay ranking ありとなしで結果を比較してみましょう。
...
```python
# Gaussian decay の最初の 3 件について詳細な内訳を追加
...
## Step 10: 時間減衰を使ったハイブリッド検索\{#step-10-hybrid-search-with-time-decay}

より複雑なシナリオでは、ハイブリッド検索を使用して dense（意味的）vector と sparse（キーワード）vector を組み合わせることができます。

```python
# ハイブリッド検索を設定（dense vector と sparse vector を組み合わせる）
dense_search = AnnSearchRequest(
    data=[query],
    anns_field="dense",  # dense vector を検索
    param={},
    limit=7
)
...
sparse_search = AnnSearchRequest(
    data=[query],
    anns_field="sparse_vector",  # sparse vector (BM25) を検索
    param={},
    limit=7
)
...
## Step 11: 異なるパラメータ値で実験する\{#step-11-experiment-with-different-parameter-values}

scale パラメータを調整すると Gaussian decay 関数にどのような影響があるかを見てみましょう。

```python
# 異なる scale パラメータを持つ Gaussian decay 関数のバリエーションを作成
...
for scale_days in [7, 14, 30]:
    scaled_ranker = Function(
        ...
        params={
            ...
            "function": "gauss",
            "origin": current_time,
            "offset": 7 * 24 * 60 * 60,  # 7 日の固定 offset
            "decay": 0.5,                # 0.5 の固定 decay
            "scale": scale_days * 24 * 60 * 60  # 可変 scale
        }
    )
...
## Step 12: 異なるクエリでテストする\{#step-12-testing-with-different-queries}

異なる検索クエリに対して decay ranking がどのように機能するかを見てみましょう。

```python
# Gaussian decay で異なるクエリを試す
...
Milvus の decay 関数を使用した時間ベースのランキングは、意味的関連性と新しさのバランスを取るための強力な方法を提供します。適切な decay 関数とパラメータを設定することで、意味的関連性を保ちながら新しいコンテンツを優先して表示する検索体験を作成できます。
...
decay 関数の背後にある数学を理解し、異なるパラメータで実験することで、特定のユースケースに対して、関連性と鮮度の最適なバランスを提供するように検索システムを微調整できます。
