...
notebook: FALSE
description: "このガイドは、Zilliz Cloud でテキストコンテンツに最適な analyzer を選択および設定するのに役立ちます。 | Cloud"
type: origin
...
このガイドは、Zilliz Cloud でテキストコンテンツに最適な **analyzer** を選択および設定するのに役立ちます。

このガイドは **実践的な意思決定** に重点を置いています。つまり、どの analyzer を使うべきか、いつカスタマイズすべきか、そして設定をどのように検証するかです。analyzer のコンポーネントやパラメータの背景情報については、[Analyzer Overview](./analyzer-overview) を参照してください。

## クイックコンセプト: analyzer の仕組み\{#quick-concept-how-analyzers-work}

analyzer はテキストデータを処理し、[full text search](./full-text-search)（BM25 ベース）、[phrase match](./phrase-match)、[text match](./text-match) などの機能で検索可能にします。生のテキストを、2 段階のパイプラインによって個別の検索可能なトークンへと変換します。

![JwMZwIYUwhbSZ4bjhxcc1PfNnvx](https://zdoc-images.s3.us-west-2.amazonaws.com/JwMZwIYUwhbSZ4bjhxcc1PfNnvx.png)

1. **Tokenization（必須）:** この初期段階では、**tokenizer** を使用して連続したテキスト文字列を、トークンと呼ばれる個別で意味のある単位に分割します。tokenization の方法は、言語やコンテンツの種類によって大きく異なる場合があります。

1. **Token filtering（任意）:** Tokenization の後、トークンを変更、削除、または精緻化するために **filters** が適用されます。これには、すべてのトークンを小文字に変換する、一般的で意味を持たない単語（stopwords など）を削除する、単語を語幹形に縮約する（stemming）といった処理が含まれます。
...
不適切な analyzer は、トークンの過剰分割または分割不足、用語の欠落、あるいは無関係な結果を引き起こす可能性があります。
...
<table>
   ...
   <tr>
     ...
     <td><p>技術用語、識別子、または URL が不適切に分割される</p></td>
   ...
   <tr>
     ...
     <td><p>句読点や空白で分割する <a href="./standard-tokenizer"><code>standard</code></a> tokenizer を使用し、カスタム <a href="./regex-filter">regex</a> filter を使います。</p></td>
   </tr>
   <tr>
     <td><p>言語の不一致</p></td>
   ...
   <tr>
     <td><p>入力方式の不一致</p></td>
     <td><p>ユーザーは Pinyin を入力するが、インデックスされたテキストは中国語の漢字を使用している。</p></td>
...
テキスト検索機能（例: **full text search**、**phrase match**、または **text match**）を使用しているが、**明示的に analyzer を指定していない** 場合、
...
この表を使って、デフォルトの [`standard`](./standard-analyzer)[ analyzer](./standard-analyzer) が要件を満たしているかをすばやく判断してください。満たしていない場合は、[別のパスを選択](./choose-the-right-analyzer-for-your-use-case#step-3-choose-your-path) する必要があります。

| コンテンツ | Standard Analyzer で十分か？ | 理由 | 必要なもの |
| --- | --- | --- | --- |
| 英語のブログ記事 | ✅ はい | デフォルトの動作で十分です。 | デフォルトを使用します（設定不要）。 |
| 中国語のドキュメント | ❌ いいえ | 中国語の単語には空白がなく、1 つのトークンとして扱われます。 | 組み込みの [`chinese`](./chinese-analyzer) analyzer を使用します。 |
| 技術ドキュメント | ❌ いいえ | `C++` のような用語から句読点が削除されます。 | [`whitespace`](./whitespace-tokenizer) tokenizer と [`alphanumonly`](./alphanumonly-filter) filter を使ってカスタム analyzer を作成します。 |
| フランス語/スペイン語など空白区切りの言語テキスト | ⚠️ 場合による | アクセント付き文字（`café` と `cafe`）が一致しない可能性があります。 | より良い結果のために、[`asciifolding`](./ascii-folding-filter) を使ったカスタム analyzer を推奨します。 |
| 多言語または不明な言語 | ❌ いいえ | `standard` analyzer には、異なる文字セットや tokenization ルールを処理するために必要な言語固有ロジックがありません。 | Unicode 対応の tokenization のために、[`icu`](./icu-tokenizer) tokenizer を使ったカスタム analyzer を使用します。<br/>あるいは、多言語コンテンツをより正確に処理するために、[multi-language analyzers](./multi-language-analyzers) または [language identifier](./language-identifier-tokenizer) の設定を検討してください。 |
...
- **パス B – カスタム analyzer を作成する**（tokenizer + filters のセットを手動で定義）

### パス A: 組み込み analyzer を使用する\{#path-a-use-built-in-analyzers}

組み込み analyzer は、一般的な言語向けに事前設定されたソリューションです。デフォルトの standard analyzer が完全には適合しない場合に、最も簡単に使い始められる方法です。
...
<table>
   ...
   <tr>
     ...
     <td><p>初期テキスト処理向けの汎用 analyzer。単一言語のシナリオでは、言語固有 analyzer（<code>english</code> など）の方がより良い性能を提供します。</p></td>
   </tr>
   <tr>
     <td><p><a href="./english-analyzer"><code>english</code></a></p></td>
     <td><p>英語専用で、stemming と stop word の削除を適用し、英語の意味マッチングを改善します</p></td>
...
[組み込み](./choose-the-right-analyzer-for-your-use-case#available-built-in-analyzers)[オプション](./choose-the-right-analyzer-for-your-use-case#available-built-in-analyzers) が要件を満たさない場合は、tokenizer と filters のセットを組み合わせてカスタム analyzer を作成できます。これにより、テキスト処理パイプラインを完全に制御できます。
...
- コンテンツに主要言語がありつつ、全体の意味への寄与が小さい外国語の単語が時折含まれる場合（例: 日本語やフランス語のブランド名や技術用語が散発的に現れる英語テキスト）。
...
[tokenizer を選択](./choose-the-right-analyzer-for-your-use-case#step-1-select-the-tokenizer-based-on-language) した後、具体的な検索要件やコンテンツ特性に基づいて filters を適用します。

##### よく使われる filters\{#commonly-used-filters}

これらの filters は、ほとんどの空白区切り言語の構成（英語、フランス語、ドイツ語、スペイン語など）で重要であり、検索品質を大幅に改善します。
...
東アジア言語（中国語、日本語、韓国語など）では、代わりに[言語固有の filters](./choose-the-right-analyzer-for-your-use-case#language-specific-filters) に注目してください。これらの言語では通常、テキスト処理に異なるアプローチが使われ、stemming の恩恵を大きく受けない場合があります。

</Admonition>

##### テキスト正規化フィルター\{#text-normalization-filters}

これらのフィルターはテキストの表記ゆれを標準化し、一貫したマッチング精度を向上させます。
...
<table>
   ...
   <tr>
     ...
     <td><p><code>jieba</code>、<code>lindera</code>、<code>icu</code> tokenizer の出力をクリーンにしたい場合。これらは句読点を単独のトークンとして返します</p></td>
   ...
   <tr>
     ...
     <td><p>英字と数字のみを保持</p></td>
     <td><p>技術文書、クリーンなテキスト処理</p></td>
   ...
   <tr>
     ...
     <td><p>ノイズを除去したい場合（過度に長いトークン）</p></td>
...
カスタム analyzer を作成するには、`analyzer_params` ディクショナリで tokenizer とフィルターのリストを定義します。フィルターは、記載された順番で適用されます。
...
analyzer を collection に適用する前に、[`run_analyzer`](https://milvus.io/api-reference/pymilvus/v2.6.x/MilvusClient/CollectionSchema/run_analyzer.md) を使ってテキスト解析のパフォーマンスをテストし、検証することをおすすめします。
...
複数の言語にまたがるコンテンツや、使用される文字体系が予測しづらいコンテンツを扱う場合は、まず `icu` analyzer から始めてください。この Unicode 対応 analyzer は、混在した文字体系や記号を効果的に処理します。
...
異なる言語間でのトークン挙動をより細かく制御するには、次を検討してください。

- **multi-language analyzer** 設定を使用する。詳細は [Multi-language Analyzers](./multi-language-analyzers) を参照してください。

- コンテンツに **language identifier** を実装する。詳細は [Language Identifier](./language-identifier-tokenizer) を参照してください。

## Zilliz Cloud で analyzer を設定してプレビューする\{#configure-and-preview-analyzers-in-zilliz-cloud}

Zilliz Cloud では、コードを書かなくても [Zilliz Cloud](https://cloud.zilliz.com/) の [console](https://cloud.zilliz.com/) から直接テキスト analyzer を設定してテストできます。
