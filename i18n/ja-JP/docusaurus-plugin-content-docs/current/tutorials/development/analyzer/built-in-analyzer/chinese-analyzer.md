...
- **Tokenizer**: `jieba` tokenizer を使用して、語彙とコンテキストに基づいて中国語テキストをトークンへ分割します。詳細については、[Jieba](./jieba-tokenizer) を参照してください。
...
`chinese` analyzer をフィールドに適用するには、`analyzer_params` 内で `type` を `chinese` に設定するだけです。
