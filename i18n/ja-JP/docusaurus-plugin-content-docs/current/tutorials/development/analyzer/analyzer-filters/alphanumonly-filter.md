...
notebook: FALSE
description: "`alphanumonly` フィルターは、非 ASCII 文字を含むトークンを削除し、英数字の用語のみを保持します。このフィルターは、基本的な文字と数字のみが関連し、特殊文字や記号を除外したいテキストの処理に役立ちます。 | Cloud"
type: origin
...
`analyzer_params` を定義した後、collection スキーマを定義する際にそれらを `VARCHAR` フィールドへ適用できます。これにより、Zilliz Cloud は効率的なトークン化とフィルタリングのために、そのフィールド内のテキストを指定した analyzer を使用して処理できます。詳細については、[Example use](./analyzer-overview#example-use) を参照してください。
