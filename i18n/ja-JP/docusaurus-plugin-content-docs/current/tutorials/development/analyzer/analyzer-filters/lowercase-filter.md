...
notebook: FALSE
description: "`lowercase` フィルターは、tokenizer によって生成された用語を小文字に変換し、検索で大文字と小文字を区別しないようにします。たとえば、`[\"High\", \"Performance\", \"Vector\", \"Database\"]` を `[\"high\", \"performance\", \"vector\", \"database\"]` に変換できます。 | Cloud"
type: origin
...
`analyzer_params` を定義した後は、collection スキーマを定義する際にそれらを `VARCHAR` フィールドへ適用できます。これにより、Zilliz Cloud は指定された analyzer を使用してそのフィールド内のテキストを処理し、効率的な tokenization と filtering を行えます。詳細については、[使用例](./analyzer-overview#example-use)を参照してください。
