...
notebook: FALSE
description: "オンデマンドクラスターのコンピュートを使ってオンデマンド検索またはクエリのワークロードを実行したい場合は、プロジェクトエンドポイントを使用します。 | Cloud"
type: origin
...
オンデマンドクラスターのコンピュートを使ってオンデマンド検索またはクエリのワークロードを実行したい場合は、プロジェクトエンドポイントを使用します。

<Admonition type="info" icon="📘" title="注">

このページは、オンデマンド検索のためにプロジェクトエンドポイントに接続する方法を説明しています。Free、Serverless、または Dedicated のサービングクラスターに接続したい場合は、[サービングクラスターへの接続](./connect-to-clusters)を参照してください。

</Admonition>

## エンドポイント形式\{#endpoint-format}

| エンドポイントの種類 | エンドポイントパターン | 使用用途 |
| --- | --- | --- |
| プロジェクトエンドポイント | `https://{project-id}.{region}.api.zillizcloud.com` | オンデマンドクラスターを介したデータインポート、バッチ検索、クエリ、get、検索、およびハイブリッド検索。 |
...
`username:password` 形式のクラスター認証情報は、サービングクラスターエンドポイント用です。プロジェクトエンドポイントを介したオンデマンド検索では、必要なプロジェクト権限を持つ API キーを使用します。

## この接続を使用するタイミング\{#when-to-use-this-connection}

バッチ処理、探索、検証、実験、および常時稼働のサービングよりもオンデマンドのコンピュートが適しているその他のワークロードには、プロジェクトエンドポイントを使用します。
