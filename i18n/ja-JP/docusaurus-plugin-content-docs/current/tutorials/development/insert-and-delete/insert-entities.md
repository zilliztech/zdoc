...
notebook: FALSE
description: "コレクション内のエンティティは、同じフィールド構成を共有するデータレコードです。各データレコードのフィールド値の集合が1つのエンティティとなります。このページでは、コレクションにエンティティを挿入する方法について説明します。 | Cloud"
type: origin
...
コレクション内のエンティティは、同じフィールド構成を共有するデータレコードです。各データレコードのフィールド値の集合が1つのエンティティとなります。このページでは、コレクションにエンティティを挿入する方法について説明します。
...
- **重複の扱い**: 標準の `insert` 操作では、主キーの重複チェックは行われません。既存の主キーを持つデータを挿入すると、同じキーの新しいエンティティが作成され、データの重複やアプリケーションの不具合を引き起こす可能性があります。既存のエンティティを更新する場合や重複を避けたい場合は、**upsert** 操作を使用してください。詳細については、[エンティティのアップサート](./upsert-entities) を参照してください。
...
コレクションにエンティティを挿入するには、そのエンティティがスキーマで定義されたすべてのフィールドを含んでいる必要があります。挿入されたエンティティは、挿入された順序で **_default** パーティションに格納されます。また、特定のパーティションが存在する場合は、挿入リクエストでパーティション名を指定することで、そのパーティションにエンティティを挿入することも可能です。

Zilliz Cloud は、コレクションのスケーラビリティを確保するために動的フィールドもサポートしています。動的フィールドを有効にすると、スキーマで定義されていないフィールドもコレクションに挿入できます。これらのフィールドと値は、**&#36;meta** という予約フィールドにキーバリューペアとして保存されます。動的フィールドの詳細については、「動的フィールド」を参照してください。
...
指定したパーティションにエンティティを挿入することもできます。以下のコードスニペットは、コレクションに **PartitionA** という名前のパーティションが存在することを前提としています。
