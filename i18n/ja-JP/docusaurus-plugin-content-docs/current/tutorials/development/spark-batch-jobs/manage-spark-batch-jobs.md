...
結果を絞り込むには、1 つ以上のオプションのフィルターを追加します。次の例では、名前が **`pk-dedup`** で始まる **`aws-us-west-2`** 内の **running** 状態の Spark バッチジョブを一覧表示します。1 ページあたり最大 **50** 件のジョブを返します。
...
オプションのフィルターは、上記のリクエストの例に示すように、同じリクエスト内で組み合わせることができます。
