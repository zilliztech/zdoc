...
クエリベクトルに最も類似したベクトルを見つけるには、近似最近傍（ANN）検索を実行します。[詳細はこちら](./single-vector-search)。
...
ベクトル検索の前にメタデータフィルタを適用して検索を絞り込み、結果の関連性を高めます。[詳細はこちら](./filtered-search)。
...
テキストや画像などを横断するマルチモーダル検索のために、複数のベクトルフィールドを組み合わせます。[詳細はこちら](./hybrid-search)。
...
ベクトル検索を行わずに、スカラーフィールドまたは主キーでフィルタリングしてエンティティを取得します。[詳細はこちら](./get-and-scalar-query)。
...
- [Dedicated](./create-cluster#create-a-dedicated-cluster)
 ...
 [どのデプロイオプションを選択すればよいかわからない場合は？](https://zilliz.com/pricing)
