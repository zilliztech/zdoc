...
beta: FALSE
notebook: FALSE
description: "Zilliz Cloud のハイライターは、テキストフィールド内の一致した用語をカスタマイズ可能なタグで囲むことで注釈を付けます。ハイライト機能は、ドキュメントがなぜ一致したのかを説明し、結果の可読性を高め、検索や RAG アプリケーションにおけるリッチなレンダリングをサポートします。| Cloud"
type: origin
