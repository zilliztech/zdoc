...
beta: FALSE
notebook: FALSE
description: "並べ替えられたベクトル埋め込みの順序を記録したインデックスファイルに基づいて、近似最近傍探索（ANN）検索は、受信した検索要求に含まれるクエリベクトルに基づいてベクトル埋め込みのサブセットを検索し、クエリベクトルとサブグループ内のベクトルを比較し、最も類似した結果を返します。ANN検索を使用すると、Zillizクラウド効率的な検索体験を提供します。このページでは、基本的なANN検索の方法を学ぶことができます。 | Cloud"
type: origin
...
並べ替えられたベクトル埋め込みの順序を記録したインデックスファイルに基づいて、近似最近傍探索（ANN）検索は、受信した検索要求に含まれるクエリベクトルに基づいてベクトル埋め込みのサブセットを検索し、クエリベクトルとサブグループ内のベクトルを比較し、最も類似した結果を返します。ANN検索を使用すると、Zillizクラウド効率的な検索体験を提供します。このページでは、基本的なANN検索の方法を学ぶことができます。

<Admonition type="info" icon="📘" title="ノート">

<p>コレクションが作成された後に新しいフィールドを動的に追加した場合、これらのフィールドを含む検索は、定義されたデフォルト値または明示的に値を設定していないエンティティの場合はNULLを返します。詳細については、<a href="./undefined">既存のコレクションにフィールドを追加する</a>を参照してください。</p>
...
kNN検索とは異なり、ANN検索アルゴリズムはベクトル埋め込みのソートされた順序を記録するインデックスファイルを要求します。検索リクエストが来た場合、インデックスファイルを参照として使用して、クエリベクトルに最も似たベクトル埋め込みを含むサブグループを迅速に検索できます。その後、指定されたメトリックタイプを使用して、クエリベクトルとサブグループの類似度を測定し、クエリベクトルとの類似度に基づいてグループメンバーをソートし、トップKグループメンバーを特定できます。

ANN検索はあらかじめ構築されたインデックスに依存し、検索スループット、メモリ使用量、検索の正確性は選択したインデックスの種類によって異なる場合があります。検索のパフォーマンスと正確性のバランスを取る必要があります。 

学習曲線を減らすために、Zillizクラウド**AUTOINDEX**を提供します。**AUTOINDEX**を使用すると、Zillizクラウドコレクション内のデータ分布を分析しながらインデックスを構築し、分析に基づいて最適化されたインデックスパラメータを設定して、検索パフォーマンスと正確性のバランスを取ることができます。 

AUTOINDEXと適用可能なメトリックタイプの詳細については、[AUTOINDEXの説明](./autoindex-explained)と[メトリックの種類](./search-metrics-explained)を参照してください。このセクションでは、以下のトピックに関する詳細情報をご覧いただけます。
...
- [使用制限とオフセット](./single-vector-search#use-limit-and-offset)
...
## シングルベクトル探索{#single-vector-search}

ANN検索では、単一ベクトル検索とは、1つのクエリベクトルのみを含む検索を指します。あらかじめ構築されたインデックスと検索要求に含まれるメトリックタイプに基づいて、Zillizクラウドクエリベクトルに最も似た上位Kベクトルを見つけます。

このセクションでは、単一ベクトル検索を実行する方法を学びます。検索リクエストには単一のクエリベクトルが含まれ、要求されますZillizクラウドクエリベクトルとコレクション内のベクトルの類似度を計算するために内積（IP）を使用し、最も類似した3つを返します。
...
同様に、検索リクエストに複数のクエリベクトルを含めることができます。ZillizクラウドクエリベクトルのANN検索を並列に実行し、2つの結果セットを返します。
...
## ANNパーティションで検索{#ann-search-in-partition}
...
次のコードスニペットは、コレクション内の**PartitionA**という名前のパーティションを想定しています。
...
検索結果には、Zillizクラウドデフォルトでは、トップKベクトル埋め込みを含むエンティティの主要なフィールド値と類似距離/スコアが含まれます。検索リクエストには、ベクトルフィールドとスカラーフィールドの両方を含むターゲットフィールドの名前を出力フィールドとして含めることができ、これらのエンティティの他のフィールドの値を検索結果に含めることができます。
...
### `count(*)`を出力フィールドとして使用する{#use-inlinecodeplaceholder0-as-output-field}

`count(*)`を出力フィールドとして使用して、コレクション内のエンティティの合計数を決定したり、フィルター条件を組み合わせて現在のコレクション内の条件を満たすエンティティの数を検索したりすることもできます。
...
検索リクエストに含まれるパラメータ`limit`が、検索結果に含めるエンティティの数を決定することに気付くかもしれません。このパラメータは、単一の検索で返すエンティティの最大数を指定し、通常は「top-K」と呼ばれます。

ページ分割クエリを実行したい場合は、ループを使用して複数の検索リクエストを送信できます。各クエリリクエストには、**Limit**および**Offset**パラメータが含まれます。具体的には、**Limit**パラメータを現在のクエリ結果に含めたいエンティティの数に設定し、**Offset**をすでに返されたエンティティの総数に設定できます。

以下の表は、一度に100個のエンティティを返す場合に、ページ分割されたクエリの**Limit**および**Offset**パラメータを設定する方法を示しています。
...
1回のANN検索での`limit`と`offset`の合計は16,384小なりになることに注意してください。
...
ANN検索を最適化するには、Zillizクラウド`level`という名前のパラメーターを提供して、単純化された検索最適化で検索精度を制御します。

このパラメータは、`1`から`10`までの範囲で、デフォルトは`1`です。値を増やすと、検索パフォーマンスが低下しながら検索再現率が向上します。一般的に、デフォルト値は最大90%の再現率をもたらします。必要に応じて値を増やすことができます。

<Admonition type="info" icon="📘" title="ノート">

<p><code>level</code>パラメータはまだ<strong>パブリックプレビュー</strong>中です。<code>5</code>と大なりな値に設定できない場合、クラスターがこの機能を完全にサポートしていない可能性があります。回避策として、<code>1</code>から<code>5</code>の範囲内の値に設定するか、<a href="https://zilliz.com/contact-sales">Zilliz Cloudサポート</a>に連絡してください。</p>
...
`level`パラメータを調整するときに、`enable_recall_calculation`を`true`に設定して、異なる`level`値で検索の精度を評価できるようにすることができます。

<Admonition type="info" icon="📘" title="ノート">

<p><code>enable_recall_calculation</code>パラメータはまだ<strong>パブリックプレビュー</strong>中であり、互換性の問題により使用できない可能性があります。サポートが必要な場合は、<a href="https://zilliz.com/contact-sales">Zilliz Cloudサポート</a>までお問い合わせください。</p>
...
AUTOINDEXはANN検索の学習曲線を大幅に平坦化します。ただし、上位Kが増えるにつれて、検索結果が常に正しいとは限りません。検索範囲を縮小し、検索結果の関連性を向上させ、検索結果を多様化することで、Zillizクラウド以下の検索強化を解決します。

- フィルター検索

    検索リクエストにフィルタリング条件を含めることができますZillizクラウドANN検索を実行する前にメタデータフィルタリングを実行し、指定されたフィルタリング条件に一致するエンティティのみに検索範囲を縮小します。

    メタデータフィルタリングとフィルタリング条件の詳細については、[フィルター検索](./filtered-search)を参照してくださいリンク_PLACEHOLDER_1これが私の人生です。

- レンジ検索

    返されたエンティティの距離やスコアを特定の範囲内に制限することで、検索結果の関連性を向上させることができますZillizクラウド範囲検索は、クエリベクトルに最も似たベクトル埋め込みを中心として2つの同心円を描画することを含みます。検索要求は、両方の円の半径を指定しますZillizクラウド外側の円に含まれるが内側の円に含まれないすべてのベクトル埋め込みを返します。
...
- キーワード一致
    ...
    キーワードマッチの詳細については、[キーワード一致](./text-match)を参照してください。
...
- パーティションキーを使う
    ...
    パーティションキーの詳細については、[パーティションキーを使う](./use-partition-key)を参照してください。

- mmapを使う

    mmap-settingsの詳細については、[mmapを使う](./use-mmap)を参照してください。

</include>

- mmapを使う

    mmap-settingsの詳細については、mmapを使うを参照してください。

- クラスタリング圧縮

    クラスタリング圧縮の詳細については、クラスタリング圧縮を参照してください。

</include>

- リランキングを使用する

    ランカーを使用して検索結果の関連性を高める方法の詳細については、[Decay Rankerの概要](./undefined)と[モデルランカーの概要](./undefined)を参照してください。
