...
beta: FALSE
notebook: FALSE
description: "ANN検索に加えて、Zillizクラウドクエリによるメタデータフィルタリングもサポートしています。このページでは、Query、Get、QueryIteratorsを使用してメタデータフィルタリングを実行する方法を紹介します。 | Cloud"
type: origin
...
ANN検索に加えて、Zillizクラウドクエリによるメタデータフィルタリングもサポートしています。このページでは、Query、Get、QueryIteratorsを使用してメタデータフィルタリングを実行する方法を紹介します。

<Admonition type="info" icon="📘" title="ノート">

<p>コレクションが作成された後に新しいフィールドを動的に追加する場合、これらのフィールドを含むクエリは、定義されたデフォルト値または明示的に値を設定していないエンティティの場合はNULLを返します。詳細については、<a href="./undefined">既存のコレクションにフィールドを追加する</a>を参照してください。</p>
...
コレクションには、さまざまな種類のスカラーフィールドを格納できます。Zillizクラウド1つ以上のスカラーフィールドに基づいてエンティティをフィルタリングします。Zillizクラウド以下の表では、Query、Get、QueryIteratorの3種類のクエリを比較しています。
...
メタデータフィルタリングの詳細については、参照してくださいリンク_PLACEHOLDER_0リンク_PLACEHOLDER_1これが私の人生です。

## Getを使用{#use-get}

主キーでエンティティを検索する必要がある場合は、**Get**メソッドを使用できます。次のコード例では、コレクションに`id`、`vector`、および`color`という3つのフィールドがあると仮定し、主キーが`1`、`2`、および`3`のエンティティを返します。
...
カスタムフィルタリング条件でエンティティを検索する必要がある場合は、**Query**メソッドを使用してください。次のコード例では、`id`、`vector`、および`color`という3つのフィールドがあると仮定し、`red`から始まる`color`値を保持する指定されたエンティティの数を返します。
...
ページ分割されたクエリを通じてカスタムフィルタリング条件でエンティティを検索する必要がある場合は、**QueryIterator**を作成し、その**next()**メソッドを使用して、フィルタリング条件を満たすエンティティを見つけるためにすべてのエンティティを反復処理します。次のコード例では、`id`、`vector`、`color`という3つのフィールドがあると仮定し、`red`から始まる`color`値を保持するすべてのエンティティを返します。
...
Get、Query、またはQueryIteratorリクエストにパーティション名を含めることで、1つまたは複数のパーティション内でクエリを実行することもできます。次のコード例では、コレクションに**PartitionA**という名前のパーティションがあると仮定しています。
...
## クエリを使用したランダムサンプリング{#random-sampling-with-query}

データ探索または開発テストのためにコレクションから代表的なデータのサブセットを抽出するには、`RANDOM_SAMPLE(sampling_factor)`式を使用します。

<Admonition type="info" icon="📘" title="ノート">

<p>詳細な使用法、高度な例、およびベストプラクティスについては、<a href="./undefined">ランダムサンプリング</a>を参照してください。</p>
