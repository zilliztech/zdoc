---
title: "Alphanumonlyという名前です。 | Cloud"
slug: /alphanumonly-filter
sidebar_label: "Alphanumonlyという名前です。"
beta: FALSE
notebook: FALSE
description: "`alphanumonly`フィルターは、非ASCII文字を含むトークンを削除し、英数字のみを保持します。このフィルターは、特殊文字や記号を除いて、基本的な文字と数字のみが関連するテキストを処理するのに役立ちます。 | Cloud"
type: origin
...
`alphanumonly`フィルターは、非ASCII文字を含むトークンを削除し、英数字のみを保持します。このフィルターは、特殊文字や記号を除いて、基本的な文字と数字のみが関連するテキストを処理するのに役立ちます。

## コンフィギュレーション{#configuration}

`alphanumonly`フィルタは組み込まれています。Zillizクラウド使用するには、`analyzer_params`内の`filter`セクションに名前を指定するだけです。
...
`alphanumonly`フィルタはトークナイザによって生成された項に作用するため、トークナイザと組み合わせて使用する必要があります。Zillizクラウド参照してくださいリンク_PLACEHOLDER_0これが私の人生です。
