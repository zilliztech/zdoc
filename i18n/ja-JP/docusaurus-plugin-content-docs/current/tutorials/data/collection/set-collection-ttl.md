---
title: "セットコレクションTTL | Cloud"
slug: /set-collection-ttl
sidebar_label: "セットコレクションTTL"
beta: FALSE
notebook: FALSE
description: "データがコレクションに挿入されると、デフォルトではそのまま残ります。ただし、特定のシナリオでは、一定期間後にデータを削除またはクリーンアップする必要がある場合があります。そのような場合は、コレクションのTime-to-Live(TTL)プロパティを設定して、ZillizクラウドTTLが期限切れになると、データが自動的に削除されます。 | Cloud"
type: origin
