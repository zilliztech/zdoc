...
このリリースでは、**Suspend**および**Resume**機能を追加することで、標準のBYOC展開オプションを使用して展開されたプロジェクトの管理も改善されています。データプレーンを一時停止して、プロジェクトに関連付けられたEC 2インスタンスとEKSクラスターを解放して、インフラストラクチャコストを削減し、必要に応じてデータプレーンを復元することができます。

今回のリリースでは、**Zilliz BYOC**の一般提供が開始され、**Zilliz BYOC-I**はプライベートプレビューとなっています。価格についてはお問い合わせください。また、ご興味のある方はこの機能をリクエストしてください。  
