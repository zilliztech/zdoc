...
notebook: FALSE
description: "Zilliz Cloud のアラート通知により、クラスター内で発生するイベントを把握できます。デフォルトでは、これらの通知は指定されたユーザーのメールアドレスに送信されますが、Webhook を使用してカスタム通知チャネルを設定し、より統合されたイベント駆動型の通知を行うこともできます。このガイドでは、アラート通知チャネルを設定する手順について説明します。 | BYOC"
type: origin
...
Zilliz Cloud のアラート通知により、クラスター内で発生するイベントを把握できます。デフォルトでは、これらの通知は指定されたユーザーのメールアドレスに送信されますが、Webhook を使用してカスタム通知チャネルを設定し、より統合されたイベント駆動型の通知を行うこともできます。このガイドでは、アラート通知チャネルを設定する手順について説明します。

## 事前準備\{#before-you-start}

通知チャネルを管理するには、[organization owner](./manage-platform-roles#predefined-organization-roles) または [project admin](./manage-platform-roles#predefined-project-roles) である必要があります。

## 通知チャネルの設定\{#set-up-notification-channels}

通知チャネルの管理ページには、Zilliz Cloud コンソールの **Edit Alert** または **Create Alert** ダイアログボックスからアクセスできます。

![manage-alert-channel](https://zdoc-images.s3.us-west-2.amazonaws.com/manage-alert-channel.png "manage-alert-channel")

### Email\{#email}

メール通知を設定する手順は以下のとおりです。
...
1. ダイアログボックスの **Send to** フィールドで、アラート通知を受信するユーザーロールまたは個別ユーザーのメールアドレスを選択します。

1. **Alert Resolution Notification** および **Enable Alert** で、アラートの解決時またはトリガー時に実行するアクションを設定します。
...
PagerDuty サービスと連携する手順は以下のとおりです。
...
1. [Zilliz Cloud コンソール](https://cloud.zilliz.com/signup) で PagerDuty 通知チャネルを設定します。
    ...
    1. 表示されるダイアログボックスの **Send to** フィールドで **+ Channel** をクリックし、ドロップダウンリストから **PagerDuty** を選択します。
...
1. [Zilliz Cloud コンソール](https://cloud.zilliz.com/signup) で Slack 通知チャネルを設定します。
    ...
    1. 表示されるダイアログボックスの **Send to** フィールドで **+ Channel** をクリックし、ドロップダウンリストから **Slack** を選択します。
...
1. Opsgenie で API キーを取得します。詳細については、[Create an API integration](https://support.atlassian.com/opsgenie/docs/create-a-default-api-integration/) を参照してください。
    ...
    1. API 設定ページで **Edit** をクリックします。デフォルトではすべての権限が選択されていますが、**Allow Read Access**、**Allow Create and Update Access**、**Allow Configuration Access** が確実に選択されていることを確認してください。
...
1. [Zilliz Cloud コンソール](https://cloud.zilliz.com/signup) で Opsgenie 通知チャネルを設定します。
    ...
    1. 表示されるダイアログボックスの **Send to** フィールドで **+ Channel** をクリックし、ドロップダウンリストから **Opsgenie** を選択します。
...
1. [Zilliz Cloud コンソール](https://cloud.zilliz.com/signup) で Lark 通知チャネルを設定します。
    ...
    1. 表示されるダイアログボックスの **Send to** フィールドで **+ Channel** をクリックし、ドロップダウンリストから **Lark** を選択します。
...
1. 作成したボットの情報を確認し、該当するボットの Webhook URL を取得します。詳しい手順については、[グループボットの Webhook アドレスの取得](https://open.work.weixin.qq.com/help2/pc/14931?person_id=1&searchData=#%E4%BA%94%E3%80%81%E7%BE%A4%E6%9C%BA%E5%99%A8%E4%BA%BAWebhook%E5%9C%B0%E5%9D%80) を参照してください。
...
DingTalk のアラート通知を設定する手順は以下のとおりです。
...
1. 作成したボットの情報を確認し、該当するボットの Webhook URL を取得します。詳しい手順については、[カスタムボットの Webhook アドレスの取得](https://open.dingtalk.com/document/orgapp/obtain-the-webhook-address-of-a-custom-robot) を参照してください。
