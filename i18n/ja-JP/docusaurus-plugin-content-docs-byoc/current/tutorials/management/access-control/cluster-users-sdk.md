---
title: "クラスターユーザーを管理する（SDK）| BYOC"
slug: /cluster-users-sdk
sidebar_label: "クラスターユーザーを管理する（SDK）"
beta: FALSE
added_since: FALSE
last_modified: FALSE
...
このガイドでは、クラスターユーザーの作成、ユーザーへのロールの付与、ユーザーからのロールの取り消し、そして最後にユーザーの削除について説明します。クラスターロールの詳細については、[クラスターロールを管理する（Console）](./cluster-roles)を参照してください。

## ユーザーを作成する\{#create-a-user}

次の例は、ユーザー名 `user_1`、パスワード `P@ssw0rd` のユーザーを作成する方法を示しています。ユーザー名とパスワードは、以下のルールに従う必要があります。

- ユーザー名: 英字で始まる必要があり、使用できるのは大文字または小文字の英字、数字、アンダースコアのみです。

- パスワード: 8～64 文字である必要があり、次のうち 3 種類を含める必要があります: 大文字、小文字、数字、特殊文字。

- （任意）説明: 最大 1024（UTF-8 バイト）。
...
以下は出力例です。一覧に `user_1` はありません。削除操作は成功しています。
