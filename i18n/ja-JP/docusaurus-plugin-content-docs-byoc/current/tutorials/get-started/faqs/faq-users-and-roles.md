...
beta: FALSE
notebook: FALSE
description: "このトピックでは、Zilliz Cloud で発生する可能性のあるユーザー、ロール、アクセスに関する問題とその解決策を紹介します。 | BYOC"
type: origin
...
このトピックでは、Zilliz Cloud で発生する可能性のあるユーザー、ロール、アクセスに関する問題と、その解決策を紹介します。
...
組織のオーナーは、自身が組織内で最後のオーナーでない場合に限り退会できます。組織には少なくとも1人のオーナーが必要であり、唯一のオーナーである場合は退会できません。
...
組織のメンバーの場合は、組織のオーナーに連絡して他のユーザーを招待してもらうよう依頼してください。
...
はい、作成可能です。まず [create a support ticket](http://support.zilliz.com) からサポートチケットを作成し、当該機能の有効化を依頼してください。機能が有効化された後、SDK を使用して設定を行えます。詳細については、[Privileges & Privilege Groups](./cluster-privileges#custom-privilege-groups) を参照してください。
