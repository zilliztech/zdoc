...
生データをコレクションにインポートする必要があるマネージドコレクションとは異なり、外部コレクションはサブ秒のリフレッシュ操作によって生データからメタデータを生成します。
...
その後、データを探索して最も価値のあるサブセットを見つけることができます。さらに、サービングクラスターに接続してデータをインポートし、本番環境で提供できます。
