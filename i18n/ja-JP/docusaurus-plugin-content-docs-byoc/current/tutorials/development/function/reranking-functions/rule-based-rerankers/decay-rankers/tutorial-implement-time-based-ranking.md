---
title: "チュートリアル: 時間ベースランキングの実装 | BYOC"
slug: /tutorial-implement-time-based-ranking
sidebar_label: "チュートリアル: 時間ベースランキングの実装"
beta: FALSE
added_since: FALSE
last_modified: FALSE
deprecate_since: FALSE
notebook: FALSE
description: "多くの検索アプリケーションでは、コンテンツの新しさは関連性と同じくらい重要です。ニュース記事、商品リスト、ソーシャルメディアの投稿、研究論文はいずれも、意味的な関連性と新しさのバランスを取るランキングシステムの恩恵を受けます。このチュートリアルでは、decay ranker を使用して Zilliz Cloud で時間ベースランキングを実装する方法を示します。 | BYOC"
type: origin
...
# チュートリアル: 時間ベースランキングの実装

多くの検索アプリケーションでは、コンテンツの新しさはその関連性と同じくらい重要です。ニュース記事、商品リスト、ソーシャルメディアの投稿、研究論文はいずれも、意味的な関連性と新しさのバランスを取るランキングシステムの恩恵を受けます。このチュートリアルでは、decay ranker を使用して Zilliz Cloud で時間ベースランキングを実装する方法を示します。

## Decay ranker を理解する\{#understand-decay-rankers}

Decay ranker を使うと、基準点に対する数値（タイムスタンプなど）に基づいてドキュメントをブーストまたはペナルティできます。時間ベースランキングでは、意味的な関連性が似ている場合でも、新しいドキュメントが古いドキュメントより高いスコアを受け取れることを意味します。

Zilliz Cloud は 3 種類の decay ranker をサポートしています。

- **Gaussian** (`gauss`): 滑らかで緩やかな減衰を提供するベル型曲線

- **Exponential** (`exp`): 初期の低下をより急にし、最近のコンテンツを強く重視
...
時間ベース検索では、コンテンツとともに公開タイムスタンプを保存する必要があります。
...
高速な vector 検索のために適切なインデックスパラメータを設定しましょう。
...
このチュートリアルでは、異なる公開日を持つニュース記事のセットを作成します。decay ranking の効果を明確に示すため、内容がほぼ同一で日付だけ異なる記事のペアを含めている点に注目してください。
...
## ステップ 5: さまざまな decay ranker を設定する\{#step-5-configure-different-decay-rankers}

次に、それぞれの違いが際立つよう、異なるパラメータを持つ 3 種類の decay ranker を作成します。
...
- `scale`: offset を超えた後に関連性がどれくらい速く低下するかを制御

- `decay`: offset+scale における減衰係数（たとえば、`0.5` は関連性が半分になることを意味します）

異なる挙動に合わせてこれらの関数を調整できることを示すため、exponential ranker には別のパラメータを設定している点に注目してください。

## ステップ 6: decay ranker を可視化する\{#step-6-visualize-the-decay-rankers}

検索を実行する前に、異なる設定の decay ranker がどのように振る舞うかを視覚的に比較してみましょう。
...
それでは、検索クエリを実行し、decay ranking ありとなしの結果を比較してみましょう。
...
より複雑なシナリオでは、ハイブリッド検索を使って dense（セマンティック）vector と sparse（キーワード）vector を組み合わせることができます。
...
## ステップ 11: さまざまなパラメータ値を試す\{#step-11-experiment-with-different-parameter-values}
...
## ステップ 12: さまざまなクエリでテストする\{#step-12-testing-with-different-queries}
...
Milvus の decay 関数を使用した時間ベースのランキングは、セマンティックな関連性と新しさのバランスを取るための強力な方法を提供します。適切な decay 関数とパラメータを設定することで、セマンティックな関連性を保ちながら新しいコンテンツを目立たせる検索体験を作成できます。

このアプローチは、特に次のような用途で有効です。
...
decay 関数の背後にある数学を理解し、さまざまなパラメータを試すことで、特定のユースケースに対して関連性と鮮度の最適なバランスを実現するよう検索システムを微調整できます。
