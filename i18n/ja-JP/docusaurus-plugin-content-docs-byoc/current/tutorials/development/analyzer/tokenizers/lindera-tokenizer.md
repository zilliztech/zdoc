...
notebook: FALSE
description: "`lindera` tokenizer は辞書ベースの形態素解析を実行します。これは日本語と韓国語向けに設計されています。これらの言語では単語がスペースで区切られず、文法上の標識（助詞）が単語に直接付加されます。 | BYOC"
type: origin
...
`lindera` tokenizer は辞書ベースの形態素解析を実行します。これは日本語と韓国語向けに設計されています。これらの言語では単語がスペースで区切られず、文法上の標識（助詞）が単語に直接付加されます。

<Admonition type="info" icon="📘" title="注意">

**中国語のテキストについて**: `lindera` は `cc-cedict` 辞書を通じて中国語もサポートしていますが、代わりに [`jieba`](./jieba-tokenizer) tokenizer の使用を推奨します。Jieba は中国語の単語分割のために特別に設計されており、より良い結果を提供します。

</Admonition>

## 概要\{#overview}

日本語と韓国語は膠着語です。助詞と呼ばれる文法標識が名詞に直接付加され、多数の組み合わせを形成します。たとえば次のようになります。
...
1. **各 token にタグ付け**し、辞書から品詞（POS）情報を付与する

1. **フィルタを適用**して不要な token（例: 助詞、句読点）を削除する

この2段階のプロセス、つまり「分割」とそれに続く「POS ベースのフィルタリング」により、検索用にどの token を index するかを正確に制御できます。

## 設定\{#configuration}

`lindera` tokenizer を使用する analyzer を設定するには、`tokenizer.type` を `lindera` に設定し、`dict_kind` で辞書を選択し、必要に応じてフィルタを適用します。
...
<table>
   ...
   <tr>
     ...
     <td><p>分割後に適用する tokenizer レベルのフィルタのリストです。各フィルタは次のオブジェクトです。</p><ul><li><p><code>kind</code>: フィルタのタイプ。サポートされる値:</p><ul><li><p><code>korean_stop_tags</code>: 指定された韓国語 POS タグに一致する token を削除します。</p></li><li><p><code>japanese_stop_tags</code>: 指定された日本語 POS タグに一致する token を削除します。</p></li></ul></li><li><p><code>tags</code>: フィルタリングする POS タグのリストです。使用可能なタグは <code>kind</code> に依存します。</p><ul><li><p><code>korean_stop_tags</code> の場合: 正確なタグコード（例: <code>JKS</code>, <code>JKO</code>, <code>SF</code>）を使用します。韓国語タグは完全一致が必要です。Sejong tagset に基づく完全な一覧については、<a href="https://docs.rs/lindera/latest/src/lindera/token_filter/korean_stop_tags.rs.html">Lindera Korean stop tags source</a> を参照してください。</p></li><li><p><code>japanese_stop_tags</code> の場合: 正確なタグコード（例: <code>助詞,格助詞</code>, <code>助詞,係助詞</code>, <code>助動詞</code>）を使用します。日本語タグは完全一致が必要です。完全な一覧（IPADIC）については、<a href="https://github.com/taku910/mecab/blob/master/mecab-ipadic/pos-id.def">Japanese POS tags reference</a> を参照してください。</p></li></ul></li></ul></td>
   </tr>
</table>

`analyzer_params` を定義した後、collection schema を定義するときにそれらを `VARCHAR` field に適用できます。これにより、Zilliz Cloud は指定された analyzer を使用してその field 内のテキストを処理し、効率的な tokenization と filtering を実行できます。詳細については、[使用例](./analyzer-overview#example-use) を参照してください。
...
`korean_stop_tags` がない場合、出力には `에서`（〜で）、`는`（主題標識）、`을`（目的格標識）などの助詞が含まれますが、これらは通常検索にはあまり有用ではありません。
...
`japanese_stop_tags` がない場合、出力には `の`（連体助詞・所有）、`は`（主題標識）、`です`（コピュラ）などが含まれます。
