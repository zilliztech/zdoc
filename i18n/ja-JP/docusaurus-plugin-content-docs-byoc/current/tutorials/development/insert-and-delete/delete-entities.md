...
notebook: FALSE
description: "不要になったエンティティは、フィルタリング条件または主キーによって削除できます。 | BYOC"
type: origin
...
不要になったエンティティは、フィルタリング条件または主キーによって削除できます。

## フィルタリング条件によるエンティティの削除\{#delete-entities-by-filtering-conditions}

複数のエンティティが共通の属性を持っている場合、それらを一括で削除するにはフィルタ式を使用できます。以下のコード例では、**in** 演算子を使用して、**color** フィールドが **red** および **purple** の値に設定されているすべてのエンティティを一括削除しています。要件に合ったフィルタ式を構築するために、他の演算子を使用することもできます。フィルタ式の詳細については、[Filtering Explained](./filtering-overview) を参照してください。
