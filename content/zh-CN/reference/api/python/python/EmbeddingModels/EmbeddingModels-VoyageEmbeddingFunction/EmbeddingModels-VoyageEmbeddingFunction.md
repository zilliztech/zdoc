...
notebook: false
description: "VoyageEmbeddingFunction 是 pymilvus 中的一个类，使用 Voyage 模型将文本编码为嵌入，以支持在 Milvus 中进行嵌入检索。 | Python"
type: docx
...
**VoyageEmbeddingFunction** 是 pymilvus 中的一个类，使用 Voyage 模型将文本编码为嵌入，以支持在 Milvus 中进行嵌入检索。
...
- **model_name** (*string*)
    ...
    用于编码的 Voyage 模型名称。您可以指定任何可用的 Voyage 模型名称，例如 `voyage-law-2`、`voyage-code-2` 等。如果您未指定此参数，则将使用 `voyage-2`。可用模型列表请参见 [Voyage 官方文档](https://docs.voyageai.com/docs/embeddings)。
...
- **kwargs**
    ...
    允许将其他关键字参数传递给模型初始化。更多信息请参见 [Python API](https://docs.voyageai.com/docs/embeddings#python-api)。
