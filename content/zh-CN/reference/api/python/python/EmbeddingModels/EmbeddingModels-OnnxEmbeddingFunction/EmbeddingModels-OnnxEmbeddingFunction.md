...
notebook: false
description: "OnnxEmbeddingFunction 是 pymilvus 中的一个类，它使用 Open Neural Network Exchange (ONNX) 嵌入模型将文本编码为嵌入向量，以支持在 Milvus 中进行嵌入检索。 | Python"
type: docx
...
OnnxEmbeddingFunction 是 pymilvus 中的一个类，它使用 Open Neural Network Exchange (ONNX) 嵌入模型将文本编码为嵌入向量，以支持在 Milvus 中进行嵌入检索。
...
- **model_name** (*string*)
    ...
    Hugging Face Hub 上包含预训练 ONNX 模型文件的仓库 ID。例如，在提供的代码中，默认设置为 `GPTCache/paraphrase-albert-onnx`。该仓库应包含与目标自然语言处理任务兼容的 ONNX 模型，例如文本分类、token 分类或特征提取。

- **tokenizer_name** (*string*)

    Hugging Face Hub 上包含与指定 ONNX 模型兼容的 tokenizer 配置的仓库 ID。在提供的代码中，默认设置为 `GPTCache/paraphrase-albert-small-v2`。tokenizer 负责文本预处理，例如分词、填充和编码，以确保与 ONNX 模型的输入格式兼容。tokenizer 应为预训练版本，并且与用于同一任务的 ONNX 模型兼容。
