...
与 [encode_documents()](./SpladeEmbeddingFunction-encode_documents) 或 [encode_queries()](./SpladeEmbeddingFunction-encode_queries) 不同，后两者允许您预先添加 **doc_instruction** 或 **query_instruction**，并使用 **k_tokens_document** 或 **k_tokens_query** 对结果进行裁剪，而 **\_\_call\_\_()** 方法会直接返回嵌入，不提供预添加指令或裁剪结果的选项。
...
- **texts** (*List[str]*)
    ...
    字符串值列表，其中每个字符串都表示将传递给嵌入模型进行编码的文本。模型会为列表中的每个字符串生成一个嵌入向量。
