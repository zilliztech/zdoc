...
notebook: false
description: "此操作向集合的 schema 添加一个字段。 | Python | MilvusClient"
type: docx
...
- **is_primary** (*bool*) -
    ...
    - 主字段的数据类型必须是 **DataType.INT64** 或 **DataType.VARCHAR**。
...
- **max_length** (*int*) -
    ...
    这是 **DataType.VARCHAR** 字段的必需参数。对于 **DataType.TEXT** 字段，请省略此参数。

- **element_type** (*str*) -

    字段值中元素的数据类型。

    这是 **DataType.ARRAY** 字段的必需参数。
...
- **dim** (*int*) -
    ...
    这是 **DataType.FLOAT_VECTOR**、**DataType.BINARY_VECTOR**、**DataType.FLOAT16_VECTOR** 或 **DataType.BFLOAT16_VECTOR** 类型字段的必需参数。如果使用 **DataType.SPARSE_FLOAT_VECTOR**，请省略此参数。

- **is_partition_key** (*bool*) -

    当前字段是否用作分区键。每个集合可以有一个分区键。
    ...
    什么是分区键？
        ...
        为便于面向分区的多租户，您可以将某个字段设置为分区键字段，以便 Zilliz Cloud 对字段值进行哈希，并据此将实体分布到指定数量的分区中。
    
        检索实体时，请确保在布尔表达式中使用分区键字段，以筛选出具有特定字段值的实体。
    
        有关详情，请参见 [使用分区键](/docs/use-partition-key) 和 [多租户](https://milvus.io/docs/multi_tenancy.md)。
...
**[CollectionSchema](./MilvusClient-CollectionSchema)** 对象，包含已添加到 schema 中的字段。
