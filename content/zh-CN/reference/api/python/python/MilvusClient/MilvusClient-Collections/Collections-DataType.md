...
notebook: false
description: "这是一个枚举，提供以下常量。 | Python | MilvusClient"
type: docx
...
- TEXT
    ...
    将数据类型设置为 **TEXT**。使用这种标量类型来存储长文本内容，例如文档、段落、工单或日志。与 **VARCHAR** 不同，TEXT 不需要 `max_length`。
...
- INT8_VECTOR
    ...
    将数据类型设置为 **Int8 Vector**。此数据类型专为量化深度学习模型（例如 ResNet、EfficientNet）设计，可在精度损失极小的情况下减小模型体积并加快推理速度。
