...
- **data** (*Union[Dict, List[Dict]]*) -<br/>
  **[必需]**<br/>
  要执行 upsert 的 Entity。必要时，可迭代输入会转换为列表。

- **timeout** (*Optional[float]*) -<br/>
  默认值：`None`<br/>
  等待 RPC 的最长时间，单位为秒。此值会覆盖客户端的默认设置。
...
- **kwargs** (*Any*) -<br/>
  其他 upsert 选项。
    ...
    - **partial_update** (*bool*) -<br/>
      默认值：`False`<br/>
      用于控制是否仅更新指定字段的标志。当该值为 `True` 时，未指定的字段保持不变。

    - **field_ops** (*Optional[Dict[str, Any]]*) -<br/>
      默认值：`None`<br/>
      部分更新期间应用的逐字段合并操作。每个值可以是 `FieldOp` 工厂结果、`array_append`、`array_remove` 或 `replace`，也可以是 `FieldPartialUpdateOp` 消息。除 `replace` 之外的任何操作都会启用部分更新。
