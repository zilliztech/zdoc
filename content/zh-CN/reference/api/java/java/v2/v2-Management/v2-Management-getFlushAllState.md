...
notebook: false
description: "此操作用于检查之前的 flush-all 操作是否已完成。当您异步调用 `flushAll` 并需要轮询完成状态时，请使用此操作。 | Java | v2"
type: docx
...
此操作用于检查之前的 flush-all 操作是否已完成。当您异步调用 `flushAll` 并需要轮询完成状态时，请使用此操作。
...
- `databaseName(String databaseName)`
    ...
    调用 `flushAll` 时使用的 Database。

- `flushAllTs(Long flushAllTs)`

    由 `flushAll` 返回的 flush-all 时间戳。
