...
notebook: false
description: "此操作会列出所有恢复快照作业。您也可以按 Collection 名称或 Database 名称进行筛选。 | Go | v2"
type: docx
...
此操作会列出所有恢复快照作业。您也可以按 Collection 名称或 Database 名称进行筛选。
...
- `WithCollectionName(collectionName string)`
    ...
    此项按目标 Collection 名称筛选恢复作业。如果未设置，则会列出所有恢复作业。

- `WithDbName(dbName string)`

    此项用于指定 Database 名称。如果未设置，则使用默认 Database。
...
RestoreSnapshotInfo 对象列表，其中每个对象都记录了一个恢复快照作业的详细信息。
