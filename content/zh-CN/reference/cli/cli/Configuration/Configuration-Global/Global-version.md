...
notebook: false
description: "此操作显示已安装的 Zilliz CLI 的版本。 | Cloud"
type: docx
...
Shell 补全会在首次运行时自动配置，并在每次升级后再次配置。CLI 会检测已安装的 shell（如 Bash、Zsh、Fish、Elvish 和 PowerShell），为 `zilliz` 和 `zz` 注册补全，并迁移由已移除的 `completion install` 命令创建的设置。
