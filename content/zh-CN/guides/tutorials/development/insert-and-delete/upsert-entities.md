...
在合并模式下，更新已有 Entity 时会保留其主键，即使 Collection 启用了 `autoID`。如果主键不存在，Zilliz Cloud 会尝试插入一个新的 Entity。插入新 Entity 时，您必须提供所有字段，否则请求会因缺少字段而失败。
...
- `upsert` 请求必须始终包含目标 Entity 的主键，即使启用了 `autoID`。对于启用 `autoID` 的 Collection，主键的处理方式取决于 upsert 模式：
    ...
    - 在合并模式下，更新已有 Entity 时会保留其主键。如果主键不存在，Zilliz Cloud 会尝试插入一个新的 Entity。插入新 Entity 时，您必须提供所有字段，否则请求会因缺少字段而失败。
