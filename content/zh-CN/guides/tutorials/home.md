...
# 从容构建，为您的 AI 应用注入强劲动力\{#build-with-confidence-and-supercharge-your-ai-applications}

Zilliz Cloud 提供完全托管的 Milvus 服务，简化向量搜索应用的部署与扩缩容，并兼顾安全性。
...
在向量搜索之前应用元数据过滤条件来缩小搜索范围，以提升结果的相关性。[了解更多](./filtered-search)。
...
按某个字段对搜索结果进行分组，以便在更高层面聚合数据，并提升结果的多样性。[了解更多](./grouping-search)。
...
组合多个向量字段，对文本、图像等进行多模态搜索。[了解更多](./hybrid-search)。
...
使用关键字匹配和 BM25 相关性评分搜索文本，实现精确的词项检索。[了解更多](./full-text-search)。
...
通过分页迭代检索超过 16,384 条上限的大规模搜索结果。[了解更多](./with-iterators)。
...
在不进行向量搜索的情况下，通过标量字段或主键过滤来检索 Entity。[了解更多](./get-and-scalar-query)。
...
选择项目[版本](./select-zilliz-cloud-service-plans)，并在该项目中创建不同部署方式的集群。
...
- [Dedicated](./create-cluster#create-a-dedicated-cluster)
 ...
 [不确定该选择哪种部署方式？](https://zilliz.com/pricing)
...
## 让计算资源靠近您的数据\{#bring-compute-resources-to-your-data}

1. 设置存储集成。

    将 AWS S3 存储桶、Google Cloud Storage 存储桶或 Microsoft Azure Blob 存储容器与 Zilliz Cloud 集成。

1. 创建 [External Volume](./external-volume)。

    将集成存储中的某个路径或整个外部存储用作 External Volume。External Volume 是对集成存储中某个存储桶或路径的只读引用，可让 Zilliz Cloud 在不复制或移动数据的情况下就地访问您的数据。

1. [创建 Database](./on-demand-database#create-database)。

    在按需计算中创建 Database。Database 是项目级资源，由项目内的所有按需集群共享。

1. 在该 Database 中创建外部 Collection。

    将 Collection 的列映射到您的 Parquet 文件、Lance 表、Iceberg 表，或 0.56.0 起的 Vortex 文件。

1. 创建索引并刷新 Collection。

    为所有向量字段和可选的标量字段建立索引，然后刷新 Collection，以便 Zilliz Cloud 为该 Collection 创建元数据和索引文件。刷新通常可在亚秒级完成。

1. 开始在您的数据中探索。

    之后，您就可以使用按需计算资源，对存储在外部存储中的数据执行向量搜索和标量过滤。

## 使用您自己的向量\{#bring-your-own-vectors}

1. 创建并连接您的集群。

    根据您所需的计算和存储资源[创建集群](./create-cluster)，然后[连接](./connect-to-clusters)该集群。

1. 创建 Collection。

    Collection 是列固定、行可变的二维数据表。您可以创建外部 Collection 将计算带到数据所在之处，也可以创建托管 Collection 以获得高性能的向量搜索服务。
...
1. 执行向量相似性搜索。
    ...
    [基础向量相似性搜索](./single-vector-search)可帮助您找到最相似的结果。
...
1. 设置模型供应商集成。
    ...
    创建 AWS、GCP 或 Azure 存储集成，用于存储第三方模型供应商的凭据。

1. 创建 Collection 并配置 Embedding Function。

    创建包含至少一个向量字段和一个 VARCHAR 字段的托管 Collection，并定义文本 Embedding Function。

1. 插入原始文本数据。

    [插入](./insert-entities)原始数据。Zilliz Cloud 会在数据写入期间自动生成向量 Embedding。

1. 使用原始文本进行搜索。

    提供原始查询文本。Zilliz Cloud 会对查询进行 Embedding，将其与已存储的向量进行比较，并[返回](./single-vector-search)最相关的结果。

## 从其他数据基础设施迁移\{#migrate-from-other-data-infra}

1. 连接到您的数据源。

    Zilliz Cloud 支持多种数据源，包括 Pinecone、MongoDB、Qdrant、PostgreSQL 等。请参见[迁移指南](./migrate-from-pinecone)。
...
1. 为您的集群或 Collection 创建备份。
    ...
    备份是集群或 Collection 在某个时间点的副本。您可以[手动](./create-backup)创建备份，也可以[设置备份策略](./schedule-automatic-backups)进行定时备份。您还可以[将备份复制到其他区域](/docs/backup-to-other-regions)，以提升容灾能力。
...
1. 恢复数据。
    ...
    在系统发生意外故障或数据丢失时[恢复您的数据](./restore-from-backup-files)。

</Stories>

<Cards>

# 进一步探索 Zilliz Cloud\{#go-further-with-zilliz-cloud}
