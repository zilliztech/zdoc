...
notebook: FALSE
description: "Schema 演进流程可以指导您在现有 Collection 中添加字段并回填数据。在此期间，您无须创建新的 Collection 或暂停生产查询。 值得注意的是，添加字段仅改变了 Collection 的数据结构。Collection 中已经存在的 Entity 在新增字段上的值仍旧为空。在不停机变更开始后，新的 Entity 和针对 Collection 中已经存在的 Entity 的更新仍可继续。 | Cloud"
type: origin
