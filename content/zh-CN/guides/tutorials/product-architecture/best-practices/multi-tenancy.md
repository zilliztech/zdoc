...
在 Database 级别的多租户模式中，每个租户对应一个独立的 [Database](./database-concept)，各个 Database 中可以包含一个或多个 Collection。
...
| --- | --- | --- | --- | --- |
| **支持的集群部署方式** | 仅 Dedicated 集群支持 | 所有部署方式 | 所有部署方式 | 所有部署方式 |
| **数据隔离** | 物理隔离 | 物理隔离 | 物理隔离 | 物理隔离 + 逻辑隔离 |
| **最多可支持的租户数量** | 1024 | 受集群部署方式和项目版本影响，最多 16,384。详见[使用限制](./limits#collections)。 | 受集群部署方式和项目版本影响，每个 Collection 中最多 1,024。详见[使用限制](./limits#collections)。 | 百万 |
| **数据 Schema 灵活性** | 高 | 中 | 低 | 低 |
| **是否支持 RBAC** | 是 | 是 | 否 | 否 |
| **搜索性能** | 高 | 高 | 中 | 中 |
| **是否支持跨租户搜索** | 否 | 否 | 是 | 是 |
| **是否支持冷热数据分层管理** | 是 | 是 | 是 | 否<br/>当前 Partition key 级别的多组策略暂不支持。如果您的租户数量极大且需要分层管理冷热数据，请[联系我们](https://zilliz.com.cn/contact-sales)。 |
