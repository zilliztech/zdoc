...
notebook: FALSE
description: "扩缩容可以帮助您在数据量、Collection 数量、流量或可用性要求增长时，保持 Dedicated Serving 集群健康运行。在 Zilliz Cloud 中，通常会因为两类原因进行扩缩容： | Cloud"
type: origin
...
## 了解扩缩容期间的计费\{#understand-billing-during-scaling}

扩缩容任务期间，Zilliz Cloud 会继续按扩缩容前的配置对集群计费。

只有扩缩容任务成功完成后，新的 Query CU 或 Replica 配置才会用于计费。该规则适用于扩容和缩容操作。

如果扩缩容任务仍在进行中或未完成，计费仍按之前的配置计算。

换句话说，在扩缩容任务成功完成之前，不会按目标配置计费。
