...
notebook: FALSE
description: "对于按需计算 Collection，DQL 操作（例如 search、query、get 和 hybrid search）需要从按需 cluster 挂载计算资源。Zilliz Cloud 允许你创建 session 来满足按需计算需求。 | BYOC"
type: origin
