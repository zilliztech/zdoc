...
notebook: FALSE
description: "Zilliz Cloud 使用身份来表示可访问组织、项目、集群、API 或 SDK 工作流的人员、组以及非人类参与者（例如服务账号）。身份管理控制哪些身份存在于 Zilliz Cloud 中，以及这些身份如何被邀请、同步、创建、移除或认证。随后，访问控制决定这些身份可以访问什么，以及可以执行哪些操作。 | BYOC"
type: origin
...
| 服务账号 | 组织或项目 | 用于应用程序、脚本、CI/CD 和自动化的非人类访问。这等同于[自定义 API 密钥](./manage-api-keys)。 |
