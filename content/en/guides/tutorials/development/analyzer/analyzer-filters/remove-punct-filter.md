...
notebook: FALSE
description: "The `removepunct` filter removes tokens that contain punctuation or whitespace from the token stream. Use it when you want cleaner text processing that focuses on meaningful content words rather than punctuation marks. | Cloud"
type: origin
