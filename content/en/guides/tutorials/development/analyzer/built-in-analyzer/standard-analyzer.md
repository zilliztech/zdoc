...
notebook: FALSE
description: "The `standard` analyzer is the default analyzer in Zilliz Cloud, which is automatically applied to text fields if no analyzer is specified. It combines the standard tokenizer with the lowercase filter. | Cloud"
type: origin
...
| --- | --- |
| `stop_words` | An array containing a list of stop words, which will be removed from tokenization. |
