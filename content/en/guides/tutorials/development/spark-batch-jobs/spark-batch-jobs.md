...
notebook: FALSE
description: "Spark batch jobs let you run distributed, offline processing on large datasets managed in Zilliz Cloud. Use built-in jobs to deduplicate, cluster, or inspect vector data, or submit a custom JAR job for application-specific processing. | Cloud"
type: origin
...
Spark batch jobs let you run distributed, offline processing on large datasets managed in Zilliz Cloud. Use built-in jobs to deduplicate, cluster, or inspect vector data, or submit a custom JAR job for application-specific processing.
...
### Built-in jobs cannot cover every workflow\{#built-in-jobs-cannot-cover-every-workflow}

Built-in jobs handle common tasks like deduplication, clustering, and anomaly detection, but real data pipelines often need business-specific rules, custom algorithms, enrichment, or downstream integration. These workflows usually require a separate Spark environment, plus connectors, dependencies, credentials, and job management.

**Custom Spark JAR jobs** let users run their own Spark applications on vector data with the same managed job lifecycle. They can package custom transformations or algorithms in a JAR, submit the job, and monitor or cancel it without running a separate Spark cluster.
...
| Run your own processing logic | [Custom Spark JAR Jobs](./custom-spark-jar-jobs) |
...
- For a custom Spark JAR job, you have uploaded the custom Spark JAR package to an accessible Zilliz Cloud Volume.
...
1. Prepare the request payload.
    ...
    - [Custom Spark JAR Jobs](./custom-spark-jar-jobs)
...
| --- | --- | --- |
| `jobName` | Yes | A descriptive name for the job to create. |
| `description` | No | An optional description for the job. |
| `projectId` | Yes | The ID of a Zilliz Cloud project in which the job runs.<br/>It is a string similar to `proj-xxxxxxxxxxxxx`. |
| `regionId` | Yes | The ID of a Zilliz Cloud region in which the job runs. For supported regions, see |
...
| `clusterSize` | No | The size of a single driver/executor. Possible values are `SMALL`, `MEDIUM`, `LARGE`, `XLARGE`, `2XLARGE`, and `3XLARGE`.<br/>~~For details about the actual specifications, refer to~~ |
| `minExecutors` | No | The minimum number of executors. The value is a positive integer ranging from `1` to `100`. |
| `maxExecutors` | No | The maximum number of executors. The value is a positive integer ranging from `minExecutors` to `100`. |
...
For a custom Spark JAR job, these two parameters are optional and depend on your application's requirements.
...
| Canceling a job | `202 ACCEPTED` | Indicates that the cancellation is accepted and underway.<br/>Canceling a job is asynchronous, and you can use the job ID carried in the response to check its progress and manage its lifecycle |
...
| `404 NOT FOUND` | Indicates that the specified resources, such as the job ID, Zilliz Cloud volume, or the custom Spark JAR package, do not exist. |
...
### Built-in and custom jobs\{#built-in-and-custom-jobs}

Zilliz Cloud provides built-in jobs for distributed, offline processing of vector embeddings, and also allows you to bring your own business logic to our Spark executors.

The following table compares the built-in jobs and custom ones.

|  | Built-in jobs | Custom JAR jobs |
| --- | --- | --- |
| Processing logic | Provided by Zilliz Cloud | Provided by the user |
| Code required | No | Yes |
| Best for | Common vector data tasks | Custom algorithms and business logic |
| Configuration | Job-specific parameters | Java/Scalar JAR, main class, arguments, and dependencies |
| Responsibility | Configure and validate results | Develop, package, and validate the application |
