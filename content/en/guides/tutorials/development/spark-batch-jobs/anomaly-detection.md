...
- If `topK` is specified, the output contains up to topK records with the highest `outlier_score` values.
...
- If `topK` was specified, the output contains at most the requested number of records.
...
To explore the broader structure of the vector data, use [K-Means Clustering](./k-means-clustering). To identify semantically redundant records, use [Vector Similarity Deduplication](./vector-similarity-dedup). 