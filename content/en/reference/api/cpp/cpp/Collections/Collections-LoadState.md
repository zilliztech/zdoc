...
notebook: false
description: "This Enum represents the load state of a collection or partition returned by `GetLoadState()`. A collection or partition must be in the `LOADSTATELOADED` state before search and query operations can be performed on it. | Cloud"
type: docx
