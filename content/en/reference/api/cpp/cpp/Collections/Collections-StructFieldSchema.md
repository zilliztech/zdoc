...
notebook: false
description: "This class describes a struct-type field (multi-vector type) in a collection schema. Pass a `StructFieldSchema` instance to `CollectionSchema:AddStructField()` when building a multi-vector schema. `StructFieldSchema` provides a fluent With/Add builder API. | Cloud"
type: docx
