...
notebook: false
description: "This class is the base class for all built-in function objects used in search reranking and full-text search. It is also used as the base for schema-level functions (e.g., BM25 tokenizer). Pass a `FunctionPtr` (a `std:sharedptr`) to `CollectionSchema::AddFunction()` or to `FunctionScore::AddFunction()`. | Cloud"
type: docx
