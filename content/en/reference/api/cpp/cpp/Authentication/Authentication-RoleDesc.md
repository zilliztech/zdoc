...
notebook: false
description: "This page documents both `RoleDesc` and `GrantItem`. `RoleDesc` represents the metadata of a Milvus role and its associated privileges. It is returned by calling `Desc()` on a `DescribeRoleResponse`. Each privilege entry is a `GrantItem` struct. | Cloud"
type: docx
