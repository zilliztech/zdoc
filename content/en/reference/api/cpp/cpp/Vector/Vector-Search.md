...
notebook: false
description: "Search a collection based on the given parameters and return results. | Cloud"
type: docx
...
- `WithFunctionChains(std::vector<FunctionChain>&& function_chains)`
    ...
    Set the function chains used to post-process search results. Function chains and rerank cannot be used together. See [FunctionChain](./Vector-FunctionChain).
...
The request accepts one query-vector representation matching the target field's [DataType](./Collections-DataType). Use the corresponding add or batch builder method; these are query inputs, not collection column payloads.
...
#### SearchResponse statistics\{#searchresponse-statistics}
