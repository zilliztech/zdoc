...
| Guides Lark source semantics, CLI entrypoint, and sidebar writer (`packages/docs-tooling/src/lark/{guidesBaseRecordSemantics,index,larkDocWriter}.*`) | `node packages/docs-tooling/src/lark/larkDocWriter.beta.test.js`, `node packages/docs-tooling/src/lark/larkDocWriter.test.js`, and relevant Lark tests | `pnpm test:replay:fetch` | `pnpm test:workflow-policy` |
