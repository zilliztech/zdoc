...
|---|---|---|---|
| Publication diagnostics, exact owned targets, and retired-manifest governance (`packages/docs-tooling/src/publication/diagnostics*`, `packages/docs-tooling/src/publication/retiredManifests*`) | `pnpm vitest run packages/docs-tooling/src/publication/diagnostics.test.ts packages/docs-tooling/src/publication/retiredManifests.test.ts`; cover identity hashing, every separately replaced path, and the finalized deprecated/non-evidence dispositions that keep retired manifests out of publication and provenance inputs | `pnpm test:replay:fetch` | `pnpm test:workflow-policy` + `git diff --check` |
| Evidence equivalence groups (`packages/docs-tooling/src/publication/evidenceGroups*`, matrix entries `evidence-fetch-publication`/`evidence-rest-derivation`/`evidence-translation-publication`, `scripts/docs-workflow/evidence-groups-contract.test.js`) | `pnpm vitest run packages/docs-tooling/src/publication/evidenceGroups.test.ts` and `node --test scripts/docs-workflow/evidence-groups-contract.test.js`; the three matrix group entries must mirror the derived path sets, and every on-disk group path must stay mapped | — | `pnpm test:workflow-matrix` + `git diff --check` |
| Translation fast-path precheck (`scripts/translation/fastpath-precheck*`, `.claude/specs/2026-09-03-translation-fastpath-design.md`) | `node --test scripts/translation/fastpath-precheck.test.js`; cover the evidence-group locale scope (ja Reference and zh Guides allowed, zh REST rejected), inventory restore, manifest regeneration, export-group derivation check, command failures, drift, and stable status codes | — | `pnpm test:workflow-matrix` + `pnpm test:workflow-policy` + `git diff --check` |
| REST OpenAPI fragments, generator, derivation evidence, and fragment migration inventory (`packages/docs-tooling/src/reference/rest/meta/openapi/**`, `fragment-migration.json`, `refGen.js`, `index.js`, `restDerivationManifest*`) | `integratedSpecBuilder`, `integratedSpecArtifacts`, `fragmentCollection`, integrated-spec CLI, completeness-receipt, and REST derivation-manifest tests | `pnpm test:rest-publication-contract` | `pnpm test:workflow-matrix` + `git diff --check` |
...
| Reference Translation manifests, targets, candidates, and evidence-chain design (`packages/docs-tooling/src/reference/translationManifest*`, `packages/docs-tooling/src/translation/{candidates*,schema.ts,targets*}`, `packages/docs-tooling/src/validation/{translation.ts,translationProvenance.test.ts}`, `scripts/translation/migrate-ja-publication-manifest*`, `deploy/evidence/ja-publication-source-checkpoints-*`, `.claude/specs/2026-09-04-publication-evidence-chains-unification-design.md`) | Focused manifest, target, and candidate tests; preserve generic Translation pairing, historical source-path provenance, retained-report receipts, REST generation ownership, cross-root landing mappings, and explicit language exclusions | — | `pnpm test:rest-publication-contract` |
...
| Shared REST reference renderer (`packages/docs-ui/src/shared/components/RestSpecs/**`) | `pnpm vitest run packages/docs-ui/src/shared/components/RestSpecs/utils.vitest.test.ts`; cover locale selection and source-language fallback for operation text | — | `pnpm test:workflow-matrix` + `git diff --check` |
| Shared docs navigation, mobile drawer, embedded demos, and document-shell layout (`packages/docs-ui/src/en/theme/Navbar/{Content,MobileSidebar}/**`, `packages/docs-ui/src/en/inkeep.config.ts`, `packages/docs-ui/src/shared/components/Supademo/**`, `packages/docs-ui/src/shared/theme/DocRoot/Layout/**`, `packages/docs-ui/src/shared/theme/DocItem/Layout/**`, `apps/docs/src/css/custom.css`) | Verify responsive control geometry, React-valid iframe markup, and that the document shell retains its bounded internal scroller | — | `pnpm test:workflow-matrix` + `git diff --check` |
| Docs UI text dictionary and its consumers (`packages/docs-ui/src/shared/i18n/**`, `packages/docs-ui/src/en/navigation/SecondaryNavbar/**`, `packages/docs-ui/src/en/theme/DocSidebar/**`) | `pnpm vitest run packages/docs-ui/src/shared/i18n/uiText.test.ts` + `pnpm vitest run packages/docs-ui/src/en/theme/DocSidebar/manualReferenceSidebar.test.ts`; cover site/locale selection (including the ja-JP locale on the English site), secondary navbar label mapping, and source-language fallback | — | `pnpm test:workflow-matrix` + `git diff --check` |
...
| Restored generated-state inventory (`scripts/restore-generated-state*`) | `node --test scripts/restore-generated-state.test.js`; the fixed path list must cover every dev-published build-provenance input (including each per-locale REST derivation manifest) or restored Fetch/Translation/verification workspaces fail closed at build time | `pnpm test:replay:all` | `pnpm test:workflow-policy` + `pnpm test:workflow-matrix` + `git diff --check` |
| Translation selection, results, reconciliation, reports, publication strategies, and the short-lock publisher (`translate-codex.yml`, `publish-translation.yml`, `_monitor-translation-progress.yml`) | `pnpm test:translation` plus the closest same-name test; split-identity changes must extend the split publisher conditional-key, monitor dual-run, recovery dual-run, and replay identity coverage | `pnpm test:replay:translation` | workflow policy + Fetch/Translation workflow contract |
...
| Requested Guides fetch planning and artifact reconstruction (`scripts/docs-workflow/guides-requested-selection*`, `scripts/docs-workflow/guides-requested-state-merge*`, `scripts/docs-workflow/guides-requested-scope*`, `packages/docs-tooling/src/lark/requestedGuidesFetchPlanner*`, `fetch-guides-requested.yml`, `_plan-guides-requested.yml`, `_fetch-guides-requested-sources.yml`) | `node --test scripts/docs-workflow/guides-requested-selection.test.js scripts/docs-workflow/guides-requested-state-merge.test.js scripts/docs-workflow/guides-requested-scope.test.js` and `node --test packages/docs-tooling/src/lark/requestedGuidesFetchPlanner.test.js`; cover selector resolution, closure/table fixpoint, scope conflicts, baseline trust, state-merge receipts, checkpoint scope allowlists, and workflow dispatch defaults | `pnpm test:replay:fetch` | workflow policy |
...
| Translation quality assets (`scripts/translation/tm-retrieve*`, `scripts/translation/prompts.js` style registration, `config/translation/ja-JP.json`, `.github/prompts/codex-style-guide.ja-JP.md`, `.claude/specs/2026-09-11-translation-quality-assets-design.md`) | `node --test scripts/translation/tm-retrieve.test.js`; cover same-directory hits, parent fallback, similarity-gated domain tier, anchor and fuzzy-anchor alignment, empty fail-closed results, few-shot rendering, and argument/path safety | none (offline asset; no workflow behavior change) | `pnpm test:translation`, `pnpm test:workflow-matrix`, and `pnpm test:workflow-policy` |
