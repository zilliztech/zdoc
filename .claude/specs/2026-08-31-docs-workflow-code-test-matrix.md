...
| Reference Translation manifests, targets, and candidates (`packages/docs-tooling/src/reference/translationManifest*`, `packages/docs-tooling/src/translation/{candidates*,schema.ts,targets*}`, `packages/docs-tooling/src/validation/{translation.ts,translationProvenance.test.ts}`, `scripts/translation/migrate-ja-publication-manifest*`, `deploy/evidence/ja-publication-source-checkpoints-*`) | Focused manifest, target, and candidate tests; preserve generic Translation pairing, historical source-path provenance, retained-report receipts, REST generation ownership, cross-root landing mappings, and explicit language exclusions | — | `pnpm test:rest-publication-contract` |
