...
| Offline Japanese Guides validation/publication: `offline-guides-publication*`, `prepare-offline-translation-publication*`, `publish-offline-translation.yml` | `node --test scripts/docs-workflow/offline-guides-publication.test.js scripts/docs-workflow/publication-coordinator.test.js`; cover exact source/execution tooling, candidate parent, target drift, cache hash, deletion/symlink/path rejection, artifact pair, staging lease, and terminal coordinator routing | `pnpm test:replay:translation` plus a real retained-artifact replay against a local bare remote | `pnpm test:workflow-policy` and scoped `actionlint` |
