...
| Offline Chinese Python Reference receipt/validation/publication: `create-offline-reference-python-receipt-artifact*`, `offline-reference-python-publication*`, `prepare-offline-reference-python-publication*`, `upload-offline-reference-python-receipt.yml`, `publish-offline-reference-python.yml` | Producer and publication tests plus publication coordinator/contracts; cover read-only receipt upload from exact immutable identities, exact one-commit candidate parent, receipt checksum and canonical source/target hashes, no deletion or out-of-Python path, trusted manifest/sidebar derivation, standard checkpoint/baseline pair, artifact-only default, and terminal coordinator routing | `pnpm test:replay:translation` plus a live-source candidate replay against a local bare remote; retain exact SHA and archive hashes | `pnpm test:workflow-policy`, Fetch/Translation workflow contract, and scoped `actionlint` |
...
| Reconciliation review, approval actions, policy exceptions, and mutation application | `pnpm test:reconciliation-workflow`; cover approve/reject/idempotency and exact plan identity | `pnpm test:replay:translation` | workflow policy + reconciliation-policy check |
...
| Guides Lark source semantics, CLI entrypoint, and sidebar writer (`packages/docs-tooling/src/lark/{guidesBaseRecordSemantics,index,larkDocWriter}.*`) | `node packages/docs-tooling/src/lark/larkDocWriter.beta.test.js` and relevant Lark tests | `pnpm test:replay:fetch` | `pnpm test:workflow-policy` |
