...
| Requested Guides fetch planning (`scripts/docs-workflow/guides-requested-selection*`, `packages/docs-tooling/src/lark/requestedGuidesFetchPlanner*`, `fetch-guides-requested.yml`, `_plan-guides-requested.yml`) | `node --test scripts/docs-workflow/guides-requested-selection.test.js` and `node --test packages/docs-tooling/src/lark/requestedGuidesFetchPlanner.test.js`; cover selector resolution, closure/table fixpoint, scope conflicts, baseline trust, and workflow dispatch defaults | `pnpm test:replay:fetch` | workflow policy |
