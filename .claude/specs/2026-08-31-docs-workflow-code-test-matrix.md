...
| Guides Lark source semantics and sidebar writer (`packages/docs-tooling/src/lark/{guidesBaseRecordSemantics,larkDocWriter}.*`) | `node packages/docs-tooling/src/lark/larkDocWriter.beta.test.js` and relevant Lark tests | `pnpm test:replay:fetch` | `pnpm test:workflow-policy` |
