...
The complete manifest is sorted by numeric track, endpoint, and method and receives a deterministic semantic SHA256. Approval of that exact grouping digest authorizes only construction of a deterministic spec execution batch. Canonical fragment and track-snapshot writes remain blocked until the user separately approves the exact execution-batch digest under the `api-reference-sync` approval contract.
...
- `latest`: load the canonical fragments and publish zdoc's current target contract without a minor-track selector. It requires the existing REST API surface selector (`v1` or `v2`) so current zdoc download files remain separate.
- `track`: load the selected Milvus minor snapshot and publish that track's current contract. It rejects the zdoc API surface selector because each Milvus minor artifact is a complete track contract.
...
`--api-version` is required for `latest` and rejected for `track`. API surface (`v1`/`v2`) and Milvus minor release track (`2.6.x`/`3.0.x`) are independent concepts and must never be inferred from the same option.
...
No existing fragment is changed when the grouping manifest is approved. That approval permits only deterministic execution-batch planning. Fragment and snapshot changes require a second approval bound to the exact current execution-batch digest. Tooling and documentation-contract implementation can be developed and tested before either data migration approval because it does not itself approve or apply spec content changes.
...
The initial branches deliver tooling and an approval-grade dry-run. Applying the proposed lifecycle metadata to all existing canonical fragments is a subsequent gated execution step bound first to the grouping digest and then to a separately approved execution-batch digest.
...
- Existing fragment migration planning remains blocked until the exact grouping digest is approved, and fragment writes remain blocked until the resulting exact execution-batch digest is separately approved.
